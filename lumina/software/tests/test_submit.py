...
def test_an_inline_vendor_is_software_scoped_and_leaves_ownership_vacant(
    ...
    # Typing a vendor's name grants no standing in it: no membership of any role, and the listing is
    # left unowned. Both are for a VendorClaim to assign, not for the person who named it inline.
    assert not VendorMembership.objects.filter(vendor=vendor).exists()
    assert Software.objects.get().owner_vendor_id is None
    assert not can_edit_listing(submitter, Software.objects.get()), (
        "the submitter has no vendor-based edit rights on the listing they named - only revise, "
        "which is keyed on the submission, lets them finish the draft"
    )
