...
def test_a_revision_restates_which_majors_it_claims(client, sent_back, reviewer):
    """``cited_releases`` is what ``approve`` certifies, so unticking a release on a bounced
    submission has to reach it. Left un-restated, the submitter corrects the form, the box
    comes back clear, and approval certifies the release anyway."""
    alma10 = AlmaLinuxRelease.objects.get_or_create(
        major=10, defaults={"supported": True})[0]
    assert [r.major for r in sent_back.cited_releases.all()] == [9]

    response = client.post(_revise_url(sent_back), {
        "software_slug": sent_back.software.slug,
        "name": "Vaultwise Archive",
        "vendor": sent_back.software.vendor.slug,
        "description": "Backs things up.",
        "homepage_url": "https://example.com/vaultwise",
        # 9 unticked, 10 ticked: "you cited the wrong release".
        "release_support_10": "on",
        "claimed_validation_level": ValidationLevel.COMMUNITY,
    })
    ...
    assert response.status_code == 302, response.context["form"].errors
    sent_back.refresh_from_db()
    assert [r.major for r in sent_back.cited_releases.all()] == [alma10.major]
