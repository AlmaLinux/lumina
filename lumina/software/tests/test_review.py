...
def submission(product, submitter):
    row = SoftwareSubmission.objects.create(
        submitter=submitter, software=product,
        claimed_validation_level=ValidationLevel.COMMUNITY,
    )
    # What the form does, and what ``approve`` reads: a submission certifies the majors it
    # cited and no others. Built by hand here, so citing has to be done by hand too - a
    # submission citing nothing certifies nothing, which is the point of the field.
    row.cited_releases.set([AlmaLinuxRelease.objects.get(major=9)])
    return row
