...
def _browse_seo(request: HttpRequest, listings) -> dict:
    """What the software browse page tells a search engine it is. See hardware's twin."""
    from lumina.core import seo

    entries = [
        (product.name, request.build_absolute_uri(product.get_absolute_url()))
        for product in listings[:50]
    ]
    return {
        "page_title": "Certified software - AlmaLinux Certification Catalog",
        "meta_description": seo.clamp(
            "Software verified to run on AlmaLinux, confirmed by its publisher, the community, "
            "or the AlmaLinux OS Foundation."
        ),
        "structured_data": seo.item_list_ld("Certified software", entries),
    }


def _detail_seo(request: HttpRequest, software, compatibility) -> dict:
    """What a product page tells a search engine it is.

    Only the majors a reader can actually see: ``_visible_compatibility`` has already dropped the
    community reports nobody has accepted, and a description naming a release the page does not
    show would be a claim the catalog has not made.
    """
    from lumina.core import seo

    url = request.build_absolute_uri(software.get_absolute_url())
    majors = sorted({row.release.major for row in compatibility})
    if (software.description or "").strip():
        description = seo.clamp(software.description)
    else:
        on = ", ".join(str(major) for major in majors)
        where = f" on AlmaLinux {on}" if on else " on AlmaLinux"
        description = seo.clamp(f"{software.name}: verified{where}.")
    return {
        "page_title": f"{software.name} - AlmaLinux Certification Catalog",
        "meta_description": description,
        "structured_data": seo.software_ld(software, url, description),
    }
...
def detail(request: HttpRequest, slug: str) -> HttpResponse:
    ...
    return render(
        ...
        {
            "software": software,
            **_detail_seo(request, software, compatibility),
            ...
            **_certify_context(software, request.user),
...
def _certify_context(software: Software, user) -> dict:
    """What the per-release row needs to offer the tier control, if it may.

    Shared by the page and the HTMX row: rendered from only one of them, a validated release
    would swap back in without its own state and the control would reappear beside the tier it
    had just recorded.
    """
    level = services.certification_level_for(user, software)
    return {
        "certifiable_as": level,
        # The vendor's own name, not "Vendor", so the button says whose validation it records.
        "certifiable_as_label": (
            software.vendor.name if level == ValidationLevel.VENDOR
            else ValidationLevel(level).label if level else ""
        ),
        "certified_majors": _certified_majors(software, level),
    }


def _certified_majors(software: Software, level: str) -> set[int]:
    """Majors already carrying ``level``, so the control does not offer what is recorded."""
    if not level:
        return set()
    return set(
        SoftwareCompatibility.objects.filter(
            software=software, certifications__level=level,
        ).values_list("release__major", flat=True)
    )
...
@login_required
@require_POST
def certify(request: HttpRequest, slug: str, major: int) -> HttpResponse:
    """One click: "my organisation validated this release". The tier half of ``attest``.

    The level comes from the clicker's standing rather than a dropdown - see
    ``services.certification_level_for`` - so the button says which level it will record and
    there is nothing to choose. A caller with no standing gets the same 404 the control's
    absence implies.
    """
    software = get_object_or_404(Software, slug=slug, published=True)
    release = get_object_or_404(AlmaLinuxRelease, major=major)
    try:
        services.certify(software=software, release=release, user=request.user)
    except ValueError as exc:
        messages.info(request, str(exc))
        return HttpResponseRedirect(software.get_absolute_url())
    return _row_response(request, software, major)
...
def propose_edit(request: HttpRequest, slug: str) -> HttpResponse:
    ...
    if request.method == "POST" and form.is_valid():
        ...
        proposal.save()
        emit("proposal.created", target=proposal, actor=request.user)
        messages.success(
...
def submit(request: HttpRequest) -> HttpResponse:
    ...
    Raising a *published* listing's tier is not this: that is one click on the release's own
    row (``certify``). This form briefly grew a ``?software=`` entry point for it, which meant
    opening the whole product form - name, publisher, description, every release - to say one
    thing about one release, and the level it carried then applied to all of them.
    """
