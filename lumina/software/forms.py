...
class SoftwareSubmissionForm(forms.Form):
    ...
    def save(self) -> SoftwareSubmission:
        ...
        self._attach_majors(software, submission)
        return submission
    ...
    def _resolve_or_create_software(self) -> Software:
        ...
        # Owned only when the submitter is acting for a vendor they represent. A vendor named inline
        # is left unclaimed - typing the name is not standing to own the listing - matching the
        # bundle path and hardware. See vendors.services.create_inline_vendor.
        owner = on_behalf
    ...
    def _sync_majors(self, software: Software) -> None:
        ...
        """
        cited = self._ticked_majors(software)
        wanted = {release.major for release in cited}
        # Restated, not added to: a revision that unticks a release is the submitter saying
        # they no longer claim it, and ``approve`` reads this list.
        self.submission.cited_releases.set(cited)
        for row in software.compatibility.select_related("release"):
    ...
    def _attach_majors(self, software: Software, submission: SoftwareSubmission) -> None:
        """Cite the ticked majors, on the listing and on the submission.

        On the listing so a reviewer sees what is being claimed, with ``get_or_create``
        because a community member may already have reported one of these majors and that
        pending row should be reused rather than collide with unique(software, release).

        On the submission because the claim belongs to the submission: ``approve`` certifies
        these and only these, so validating one release cannot raise the others.
        """
        submission.cited_releases.set(self._ticked_majors(software))

    def _ticked_majors(self, software: Software) -> list:
        """The releases whose boxes are ticked, each with a row on the listing."""
        cited = []
