...
class SoftwareSubmission(ReviewWorkflow, models.Model):
    ...
    # Which AlmaLinux majors this submission actually claims, recorded when it is made.
    #
    # ``approve`` used to walk the listing's own compatibility rows, which was correct only
    # while every one of them came from this submission - true for a brand-new product and
    # false the moment a *published* listing can be submitted again. Re-validation makes that
    # reachable: somebody validating one major would have had every major the listing ever
    # carried raised to their level, on evidence for one of them. Hardware carries the same
    # field for the same reason; see ``hardware.models.Submission.cited_releases``.
    cited_releases = models.ManyToManyField(
        "releases.AlmaLinuxRelease", blank=True, related_name="software_submissions",
    )
    ...
    def approve(self, *, by, final_level: str) -> None:
        ...
        Only the majors this submission cited. It used to walk every compatibility row the
        listing had, which was the same defect hardware's ``cited_releases`` was added to fix:
        correct while a submission could only ever be a new product, and wrong as soon as a
        published listing can be submitted again, because validating one release would raise
        every release the listing had ever carried.
        ...
        rows = list(self.software.compatibility.filter(
