...
from lumina.results import device_inventory
...
# System-identity fields that must never reach the public: the SMBIOS UUID and
# firmware serials, which name a specific physical unit. Nothing in lumina reads
# them (dedup keys on vendor+product, see identity_key), so removing them from a
# public view changes no behavior. Kept as a module constant so the survey and
# any future scrubber agree on the exact set.
PUBLIC_IDENTITY_STRIP = {
    "system": ("uuid", "serial"),
    "baseboard": ("serial",),
    "chassis": ("serial", "asset_tag"),
}


def public_inventory(inventory: dict) -> dict:
    """A deep copy of ``inventory`` with system-identity fields removed.

    The public run-detail API returns the inventory blob; these keys are a
    machine's serials and SMBIOS UUID, which identify one physical unit and are
    never shown publicly. The stored blob is left untouched - only this view of
    it is scrubbed - and the survey's access-controlled table is the only place
    raw identity is retained.
    """
    clean = copy.deepcopy(inventory or {})
    summary = clean.get("summary")
    if isinstance(summary, dict):
        for section, keys in PUBLIC_IDENTITY_STRIP.items():
            block = summary.get(section)
            if isinstance(block, dict):
                for key in keys:
                    block.pop(key, None)
        # /etc/machine-id sits at the top of summary, not in a DMI section - the OS
        # install identity, also never public.
        summary.pop("machine_id", None)
    return clean
...
def identity_key(vendor, product) -> tuple[str, str]:
    """A reported (vendor, product) pair normalized for same-machine comparison: each half stripped
    and lower-cased, so firmware capitalization or whitespace changes compare equal. The DB-side
    twin is ``__iexact`` on each half (see ``reported_identity_pairs`` / ``find_matching_system``)."""
    return ((vendor or "").strip().lower(), (product or "").strip().lower())


# --- generic product lines -------------------------------------------------------
#
# Distinct from a placeholder: a real, non-empty Product Name that names a product *line* rather
# than a model - Supermicro "Super Server", where the actual model lives on the motherboard. Such a
# string must never key a System listing, or every model in the line collapses onto whichever one
# was catalogued first. Which strings are generic is admin-editable (the ``GenericModel`` table,
# seeded with the known offenders); the set is cached because ``is_generic_model`` is consulted on
# hot paths such as ``effective_system_kind``, and invalidated by a signal when the table changes.
GENERIC_CACHE_KEY = "results:generic-models"


def _generic_pairs() -> frozenset:
    from django.core.cache import cache

    pairs = cache.get(GENERIC_CACHE_KEY)
    if pairs is None:
        from lumina.results.models import GenericModel

        pairs = frozenset(
            identity_key(row.vendor, row.product)
            for row in GenericModel.objects.all()
        )
        cache.set(GENERIC_CACHE_KEY, pairs, 3600)
    return pairs


def invalidate_generic_cache(*args, **kwargs) -> None:
    from django.core.cache import cache

    cache.delete(GENERIC_CACHE_KEY)


def is_generic_model(vendor, product) -> bool:
    """Whether ``(vendor, product)`` is a generic product line, not a specific model.

    A ``GenericModel`` row with a blank vendor matches any manufacturer; matching is
    case-insensitive on both halves. Placeholder junk is handled by ``is_placeholder`` and is not
    repeated here - this is only the "real string, still not a model" case.
    """
    if not product:
        return False
    v, p = identity_key(vendor, product)
    pairs = _generic_pairs()
    return ("", p) in pairs or (v, p) in pairs
...
def extract(inventory: dict) -> dict:
    ...
    # Through the categorizer, not straight off ``summary["gpus"]``: that list is the
    # collector's own older categorization and comes back empty whenever the GPU collector
    # errored, even though ``pci_devices`` still enumerates the card. Reading it directly
    # meant a run whose device table showed an RTX card reported no GPU in its own
    # columns. One definition of "this is a GPU", in device_inventory.
    gpus = device_inventory.categorize_inventory(inventory or {})["gpus"]
    ...
    # Deferred like the other catalog imports in this module, to keep import order free
    # of a models dependency at module load.
    from lumina.results.component_match import normalize_cpu_vendor

    return {
        "cpu_model": _clean(cpu.get("model"), 200),
        # ``cpu_model`` stays verbatim because ``component_match`` normalizes it downstream
        # on its way into the catalog. The vendor has no such downstream - this column is
        # what gets displayed and filtered on - so it is tidied here or nowhere, which
        # matters on Arm, where it arrives from the firmware as "Ampere(R)".
        "cpu_vendor": _clean(normalize_cpu_vendor(cpu.get("vendor") or ""), 80),
