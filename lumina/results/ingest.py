...
# The major version this server understands. The schema's own contract, which both projects are
# written against, is that "additive optional fields bump the minor version, breaking changes bump
# the major version" - so a minor is by definition something an older reader can ignore.
#
# This was a set of exact versions, and that made every additive release of the suite a production
# outage here: 1.2 added ``summary.fields`` and nothing else, and every submission from a suite
# carrying it was refused until somebody edited a Python set and deployed. Refusing a *major* is
# the check the contract actually asks for, and it is the one that fails only when something
# really did break.
...
def ingest_bundle(
    *,
    ...
    submitter_notes: str = "",
    publish_anonymously: bool | None = None,
) -> TestRun:
    ...
    with tempfile.TemporaryDirectory(dir=tmp_root, prefix="ingest-") as tmpdir:
        ...
        # Untick, by default, the devices an exclusion rule matches (a BMC display adapter, an
        # onboard iGPU), recording why. Before any review, so it seeds a default a reviewer can
        # override. Deferred import: services depends on this app's models.
        from lumina.results.services import seed_rule_exclusions

        seed_rule_exclusions(run)
    ...
    # Fork a survey record from this run's inventory - the census stream, independent of
    # certification. Deferred import; isolated so a survey-side failure never breaks the ingest.
    from lumina.survey.services import fork_survey_record

    fork_survey_record(run)

    # A validation run lands as a draft its submitter must add details to and submit; tell them.
    if run.status == TestRun.STATUS_DRAFT:
        emit("run.needs_details", target=run, actor=submitter)
    return run
...
def check_schema_version(report: dict) -> None:
    """Refuse a report this server cannot read. Shared with the survey's own validator."""
    version = report.get("schema_version")
    major = str(version).split(".")[0] if version is not None else ""
    if major != SUPPORTED_SCHEMA_MAJOR:
        raise UnsupportedSchema(
            f"schema_version {version!r} is not supported; "
            f"this server accepts {SUPPORTED_SCHEMA_MAJOR}.x."
        )
...
def _validate_report(report: dict) -> None:
    ...
    # It belongs at the survey endpoint; this one certifies nothing and would only sit
    # unreviewable. (``collect`` remains valid as the first phase of a validate/benchmark
    # run, where the primary type is validate/benchmark and this does not fire.)
    if _primary_run_type(run_types) == RunType.collect.value:
        raise InvalidReport(
            "A collect-only run is a hardware-survey submission. Upload it to "
            "/api/v1/survey/, not /api/v1/results/."
        )
...
def _create_run(
    *,
    ...
    # run metadata and as a submit field, so ``alma-certify submit`` can correct a value the run
    ...
    if not TestRun.environment_is_prerelease_os(environment):
    ...
    run = TestRun(
        ...
        publish_anonymously=_effective_anonymity(submitter, publish_anonymously),
...
def _effective_anonymity(submitter, requested: bool | None) -> bool:
    """Whether this run publishes without a username.

    ``requested`` is what the upload said and wins outright, in both directions: a run
    submitted with ``--anonymous`` is anonymous even from an account that normally
    publishes under its name, and a run that explicitly asked to be attributed stays
    attributed even from an account whose default is anonymous. Only when the upload
    said nothing at all does the account default decide - which is why the client sends
    the field only when it has an answer rather than always sending a bool.
    """
    if requested is not None:
        return bool(requested)
    return AccountSettings.publishes_anonymously(submitter)
...
def _create_results(run: TestRun, report: dict) -> None:
    ...
    # The run's inventory GPUs carry the PCI ids a clpeak benchmark row does not, so each GPU row is
    # tied back to its card here to get a backend-independent grouping key. Read once for the run.
    inventory_gpus = (run.inventory.get("summary") or {}).get("gpus") or []
    ...
    for entry in report["results"]:
        ...
        for metric in entry.get("metrics") or []:
            ...
            # The card this ran on, as a "vendor:device" PCI key, so a card's Vulkan and OpenCL
            # figures group as one GPU rather than as two device names. Blank when it cannot be
            # pinned down; grouping falls back to device_model there.
            device_pci_id = benchmark_gpu_pci_id(device_raw, inventory_gpus) if device_raw else ""
            benchmarks.append(
                BenchmarkResult(
                    run=run,
