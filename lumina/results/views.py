...
from lumina.results import benchmark_registry, compare, gpu_metrics, ingest, services
    ...
    if request.method == "POST" and form.is_valid():
        try:
            run = ingest.ingest_bundle(
                ...
                # ``or None`` so an unticked box leaves the account default in charge,
                # rather than overriding it with an answer the uploader never gave.
                publish_anonymously=form.cleaned_data["publish_anonymously"] or None,
            )
...
def run_detail(request: HttpRequest, uuid) -> HttpResponse:
    ...
    return render(
        ...
        "results/run_detail.html",
        {
            ...
            # Only the submitter, and only on the public page: the review page includes the
            # same summary and must show the real name with no control over it.
            "show_anonymity_toggle": run.submitter_id == getattr(request.user, "pk", None),
            ...
            # The vendor this run's own submitter may now re-assess it under, or None. Set only when
            # they have since joined the verified vendor that owns the listing (see reassessable_as).
            "reassessable_vendor": services.reassessable_as(run, request.user),
        },
    )
...
@login_required
@require_POST
def reassess_run(request: HttpRequest, uuid) -> HttpResponse:
    """Re-assess the submitter's own run under the verified vendor they have since joined.

    Self-service: eligibility is ``reassessable_as`` and the service re-checks it, so this needs no
    reviewer - the vendor tier is already this person's to claim on that listing.
    """
    run = get_object_or_404(TestRun, uuid=uuid)
    vendor = services.reassessable_as(run, request.user)
    if vendor is None:
        return HttpResponseForbidden("This run is not yours to re-assess as a vendor.")
    services.reassess_run_as_vendor(run, vendor=vendor, by=request.user)
    messages.success(
        request,
        f"Re-assessed as {vendor.name}. Your evidence now counts at the vendor tier on the "
        f"listings {vendor.name} owns.",
    )
    return redirect("results:run_detail", uuid=run.uuid)
...
def _generic_reported(run) -> bool:
    """Whether the machine's firmware reported a generic product line, not a model.

    Distinguishes a Supermicro "Super Server" from a genuine self-build: both are identified by
    their board, but the generic case is asked whether a real vendor model exists that the firmware
    did not report.
    """
    from lumina.results.inventory_extract import is_generic_model

    return is_generic_model(run.system_vendor, run.system_product)
...
def _listing_prompt(user, run) -> dict | None:
    ...
    if existing is None:
        ...
        return {"kind": kind, "subject": subject,
                "generic_reported": _generic_reported(run)}
...
def propose_listing(request: HttpRequest, uuid) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # Still written, and now carrying only the per-part vendor claim: the submitter's
            # brand and model boxes are gone, so this form contributes no naming correction to it.
            # The claim is theirs to make and lives here, keyed the same way.
            run.component_overrides = form.component_overrides()
            # What the submitter said a part really *is*. A suggestion, not a correction: it names
            # nothing until a reviewer acts on it, which is why it is kept apart from the
            # overrides above. Their free text became the catalog component's name, so a typo
            # minted a catalog entry; a value their own machine reported cannot.
            run.component_suggestions = {
                **(run.component_suggestions or {}), **form.component_suggestions(),
            }
            ...
            # Whose name the published run carries. Arrives at ingest from ``--anonymous`` or
            # the account default, is answered here, and stays changeable from the run page
            # afterwards - the submitter owns this one at every point.
            run.publish_anonymously = bool(cleaned.get("publish_anonymously"))
            ...
            run.save(update_fields=["listing_proposal", "on_behalf_of",
                                    ...
                                    "available_from_minor", "publish_anonymously"])
            ...
            # "Save and submit for review", the second button. Everything above ran either
            # way, so a failed submission still keeps the answers: the details are saved
            # before this is looked at, and a run that cannot go to the queue yet is not a
            # reason to throw away what the submitter just typed.
            if "save_and_submit" in request.POST:
                try:
                    services.submit_for_review(run, by=request.user)
                except services.ReviewError as exc:
                    # Back to this form rather than the run page. For a validation run that
                    # is not scoped, everything ``missing_submission_details`` asks for is a
                    # field of this form, so this is where the submitter can act on it.
                    messages.error(request, f"Saved, but not submitted. {exc}")
                    return HttpResponseRedirect(
                        reverse("results:propose_listing", args=[run.uuid])
                    )
                messages.success(
                    request,
                    "Listing details saved and the run submitted for review. "
                    "A reviewer will look at it shortly.",
                )
            else:
                messages.success(
                    request,
...
@login_required
@require_POST
def set_run_anonymity(request: HttpRequest, uuid) -> HttpResponse:
    """Toggle whether one of the submitter's own runs is published anonymously.

    Redirects back to where it was pressed, like the archive controls beside it: this is
    reachable from the dashboard run tables and from the run page.
    """
    run = get_object_or_404(TestRun, uuid=uuid, submitter=request.user)
    anonymous = request.POST.get("anonymous") == "true"
    try:
        services.set_run_anonymity(run, by=request.user, anonymous=anonymous)
    except services.ReviewError as exc:
        messages.error(request, str(exc))
    else:
        messages.success(
            request,
            "This run is now listed anonymously."
            if anonymous
            else f"This run is now listed as {request.user.get_username()}.",
        )
    return HttpResponseRedirect(_back_to(request))
...
# The sortable score columns per hardware kind. CPU alone carries the single/multi split.
_SCORE_COLUMNS = {"cpu": ("overall", "single", "multi"), "gpu": ("overall",)}


def benchmark_scores(request: HttpRequest) -> HttpResponse:
    """The overall leaderboard: hardware ranked by composite, relative Mark.

    A per-benchmark leaderboard answers "which is fastest at sysbench"; this answers "which is
    fastest, overall", combining every comparable benchmark into one relative score per model (see
    ``benchmark_scoring``). CPU and GPU are separate boards - the ``kind`` tab - so neither list is
    diluted by the other and each stays a page or two. The score columns sort server-side (so the
    order is over the whole board, not just the page shown), and the board paginates.
    """
    from django.core.paginator import Paginator

    from lumina.results import benchmark_scoring

    kind = "gpu" if request.GET.get("kind") == "gpu" else "cpu"
    board = (
        benchmark_scoring.gpu_leaderboard() if kind == "gpu"
        else benchmark_scoring.cpu_leaderboard()
    )

    sortable = _SCORE_COLUMNS[kind]
    sort = request.GET.get("sort")
    if sort not in sortable:
        sort = "overall"
    descending = request.GET.get("dir", "desc") != "asc"

    # None (e.g. a CPU with no single-core benchmark) sorts last whichever way the column runs.
    present = [entry for entry in board if entry.get(sort) is not None]
    absent = [entry for entry in board if entry.get(sort) is None]
    present.sort(key=lambda entry: entry[sort], reverse=descending)
    board = present + absent

    header = {
        key: {
            "dir": "asc" if key == sort and descending else "desc",
            "arrow": ("↓" if descending else "↑") if key == sort else "",
        }
        for key in sortable
    }

    page = Paginator(board, 100).get_page(request.GET.get("page"))
    return render(
        request,
        "results/benchmark_scores.html",
        {
            "kind": kind,
            "page": page,
            "header": header,
...
def leaderboard(request: HttpRequest, benchmark_id: str) -> HttpResponse:
    ...
    """
    # Lumina decides what has a leaderboard, not what was submitted: a benchmark not in the registry
    # (retired, or one lumina was never taught about) has no page, even if old rows still carry its
    # id.
    if not benchmark_registry.is_listed(benchmark_id):
        raise Http404("Not a listed benchmark.")
    ...
    if version is None:
        # A listed benchmark nobody has run yet: its page exists (it is one lumina defines), it just
        # shows an empty state rather than a 404.
        return render(
            request,
...
def stats(request: HttpRequest) -> HttpResponse:
    """The public hardware statistics page: the AlmaLinux hardware survey.

    Reads the survey rollup and nothing else. It used to also carry aggregates over
    published certification runs, which were removed: they answered a different question
    ("what has been certified") on a different population, they could not carry a share
    over time because a certified machine never reports again, and putting the two on one
    page invited a comparison neither of them supports.

    The view is in ``results`` because that is where the URL has always been. The reading
    itself belongs to the survey app, which owns its own rollup.
    """
    from lumina.survey import stats as survey_stats

    return render(request, "results/stats.html", survey_stats.page_context(request.GET))
