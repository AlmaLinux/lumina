...
A ``TestRun`` is one execution of the alma-certify suite (collect, validate, or
...
class TestRunQuerySet(models.QuerySet["TestRun"]):
    ...
    def with_benchmarks(self) -> TestRunQuerySet:
        ...
        list, so ``_primary_run_type`` collapses it: a combined ``alma-certify run``
...
class TestRun(models.Model):
    ...
    # Whether the *public* sees a username against this run. The submitter is still
    # recorded above and is still shown to reviewers and admins, who need to know whose
    # evidence they are judging: this hides attribution, it does not anonymize the
    # submission. Set at ingest from the ``--anonymous`` flag or the account default,
    # correctable on the proposal form, and toggleable by the submitter afterwards -
    # somebody who contributed a run before deciding they would rather not be named,
    # or the reverse, should not have to ask anyone.
    ...
    # Why a tie is excluded *by an automatic rule*, as ``{tie_key: reason}`` - a BMC display
    # adapter, an onboard iGPU nobody certifies. Seeded at ingest from ``ComponentExclusionRule``
    # and the categorical non-accelerator rule; the matched keys are also seeded into
    # ``excluded_component_ties`` so they arrive unticked, and this holds the reason to show beside
    # them ("excluded per rule: ..."). A reviewer ticking one to include it removes it from
    # ``excluded_component_ties`` (a human decision, which then sticks); this map is left as-is, so
    # it only ever explains why something *is* unticked. Empty on runs before the rules existed.
    component_exclusion_reasons = models.JSONField(default=dict, blank=True)
    ...
    # What the submitter said a part really is, as ``{tie_key: {"path": ..., "value": ...}}``.
    #
    # Kept apart from ``component_overrides`` deliberately: that is what a reviewer decided and it
    # names the catalog entry, this is what somebody holding the machine offered and it names
    # nothing until a reviewer acts on it. Blurring the two is how a suggestion becomes a catalog
    # component under a string nobody checked.
    #
    # The ``path`` is where on this machine the value came from, which the submitter never sees
    # and never types - they pick from values their own machine reported. It is the half that
    # makes the answer worth more than a note: a reviewer turns it into a naming rule that fixes
    # every machine of that shape rather than this one run.
    component_suggestions = models.JSONField(default=dict, blank=True)
    ...
    def notification_fields(self) -> list[tuple[str, str]]:
        """Labelled facts for a rich chat notification.

        Read by ``notifications.payloads.target_fields`` if it exists, so this is the
        model saying what it is worth showing rather than the notifications app
        type-sniffing every object it might be handed. Enough to recognise the run
        without opening it, and nothing access-controlled: a chat channel is a wider
        audience than a review page.
        """
        machine = " ".join(
            part for part in (self.system_vendor, self.system_product) if part
        ) or self.board_model or "unidentified machine"
        return [
            ("Machine", machine),
            ("CPU", self.cpu_model),
            ("GPU", self.gpu_model),
            ("AlmaLinux", str(self.alma_release) if self.alma_release_id else ""),
            ("Run", self.get_run_type_display()),
            ("Submitted by", self.public_submitter_name),
        ]
    ...
    @property
    def public_submitter_name(self) -> str:
        """Who the public is told submitted this run.

        Every public surface must go through this rather than reaching for
        ``submitter.get_username`` - a template, a serializer, or a feed that reads the
        username directly is a leak, and it looks exactly like correct code. Reviewer
        and submitter surfaces read ``submitter`` deliberately and directly: the point
        of the flag is that a reviewer still knows whose evidence they are judging.
        """
        if self.publish_anonymously:
    ...
    def cpu_flag_groups(self) -> dict:
        ...
        suite already makes (``NOTABLE_FLAGS`` in ``alma_certify/validate/cpu.py``);
    ...
    def ran_on_prerelease_os(self) -> bool:
        ...
        """
        return self.environment_is_prerelease_os(self.environment)

    @staticmethod
    def environment_is_prerelease_os(environment) -> bool:
        """Whether a raw environment blob reports AlmaLinux Kitten.

        The pre-row twin of ``ran_on_prerelease_os``: ingest needs this same reading before a
        ``TestRun`` exists, and the two must not drift, so both go through here. Kitten names itself
        only in ``PRETTY_NAME``; every other field matches the stable release of the same major.
        """
    ...
    def reported_identity_pairs(self) -> list:
        ...
        pairs = []
        # A generic product line ("Super Server") is not an identity: aliasing or matching on it
        # would attach every model in the line to whatever the first one produced. Skipped here so
        # the board becomes the machine's identity, exactly as a placeholder system string does.
        if (self.system_product or "").strip() and not inventory_extract.is_generic_model(
            self.system_vendor, self.system_product
        ):
    ...
    def effective_system_kind(self) -> str:
        ...
        # A generic product line (Supermicro "Super Server") names no model, so the board is the
        # identity - the same treatment a placeholder gets, but for a real string. Interpreted here
        # rather than frozen at ingest, so it applies to bundles already collected without a re-run,
        # and the submitter/reviewer overrides above still win.
        from lumina.results import inventory_extract
        if inventory_extract.is_generic_model(self.system_vendor, self.system_product):
            return SystemKind.CUSTOM
        return self.system_kind

    @property
    def is_custom_build(self) -> bool:
        """Whether the motherboard - not a vendor system model - is this machine's identity.

        True when the effective kind is a custom build, whether because the machine reported no
        vendor system model or because it reported a generic product line ("Super Server").
        ``effective_vendor``, ``effective_product`` and the display name all branch on this, so the
        concept is named once rather than spelled as a raw enum comparison in each.
        """
        return self.effective_system_kind == SystemKind.CUSTOM
    ...
    @property
    def identity_overridden(self) -> bool:
        """Whether the submitter named the machine something the firmware did not report.

        A reviewer should look harder when a human has supplied a model the run did not detect - a
        corrected name, or a real vendor model given for a machine whose firmware reported only a
        generic line. Not flagged when they accepted or picked a detected string, or supplied none.
        Computed from denormalized fields only, so the review queue can show it per row with no
        extra query.
        """
        stated = ((self.listing_proposal or {}).get("name") or "").strip().lower()
        if not stated:
            return False
        detected = {
            (self.system_product or "").strip().lower(),
            (self.board_model or "").strip().lower(),
        }
        detected.discard("")
        return stated not in detected
    ...
    def verdict(self) -> bool | None:
        ...
        True when no result blocks. A blocking result is a gating validation
        (REQUIRED/CONDITIONAL) that failed and no reviewer has waived: informational
        tests and benchmarks (which carry no severity) never gate, and a reviewer may
        waive a specific failure so the run certifies in spite of it. Either way the
        result is still stored and shown - it is only not counted here.
        """
        ...
        if "results" in prefetched:
            ...
            return not any(result.is_blocking for result in prefetched["results"])
        return not self.results.blocking().exists()
...
# Severities that make a result a gating validation - a REQUIRED or CONDITIONAL test is a pass/fail
# check that certification stands on. INFORMATIONAL is evidence a reviewer reads, and a benchmark
# carries no severity at all (it measures, it does not check). Neither of those gates a run, and
# neither counts as the passing evidence a scoped claim needs. Defined once so "what is a validation"
# cannot drift between the blocking-failure rule and the evidencing-pass rule.
GATING_SEVERITIES = (Severity.REQUIRED, Severity.CONDITIONAL)


class TestResultQuerySet(models.QuerySet):
    def gating(self) -> TestResultQuerySet:
        """Results whose severity makes them a validation certification stands on (REQUIRED or
        CONDITIONAL). Informational tests and benchmarks (no severity) are excluded."""
        return self.filter(severity__in=GATING_SEVERITIES)

    def blocking(self) -> TestResultQuerySet:
        """Gating validations that failed and no reviewer has waived - the results that stop a run
        certifying. A reviewer waives one by setting ``waived_by``, which lets approval certify the
        system despite it."""
        return self.gating().filter(
            status__in=[ResultStatus.FAIL, ResultStatus.ERROR], waived_by__isnull=True
        )

    def waived(self) -> TestResultQuerySet:
        """Failing results a reviewer has waived, shown on the run as certified-despite."""
        return self.filter(waived_by__isnull=False)
...
class TestResult(models.Model):
    ...
    # A reviewer may waive a single failing result so the run certifies in spite of it - the machine
    # is fine, the failure is understood, and re-running to launder a known-benign error would be
    # theatre. Recorded on the result, with who and why, because certifying hardware over a failure
    # is a decision that has to carry an attributable reason.
    ...
    objects = TestResultQuerySet.as_manager()
    ...
    def is_waived(self) -> bool:
        return self.waived_by_id is not None

    @property
    def is_blocking(self) -> bool:
        """Whether this result stops the run certifying: a gating validation (REQUIRED/CONDITIONAL)
        that failed and no reviewer has waived. The in-memory twin of ``TestResultQuerySet.blocking``,
        for ``verdict``'s prefetched-results path."""
        return (
            self.severity in GATING_SEVERITIES
            and self.status in (ResultStatus.FAIL, ResultStatus.ERROR)
            and self.waived_by_id is None
        )
...
class BenchmarkResult(models.Model):
    ...
    # device_model is lumina's canonicalization of it (via normalize_gpu_model) and is the human
    # label. device_pci_id is the "vendor:device" of the inventory card the row ran on (e.g.
    # "8086:7d55"), resolved at ingest, and is the grouping key the leaderboard and compare page
    # actually key on: it is the same for a card whatever backend named it, so Vulkan's
    # "Intel Graphics (ARL)" and OpenCL's "Intel Graphics" group as one GPU. It is blank whenever
    # the card could not be pinned down (an older bundle, or more than one different GPU of that
    # vendor in the run), and every grouping site falls back to device_model when it is - which is
    # exactly the pre-PCI behaviour. All three are blank on non-GPU metrics and on pre-device-field
    ...
    # device_pci_id is deliberately NOT in the unique key: two identical cards share it, and
    # device_ordinal is what separates their rows.
    ...
    device_pci_id = models.CharField(max_length=32, blank=True, db_index=True)
...
class GenericModel(models.Model):
    """A DMI system-product string that names a product *line*, not a specific model.

    Some vendors ship a whole range under one Product Name - Supermicro's "Super Server" is the
    canonical case, where the real model lives on the motherboard. Keying a System listing on
    (vendor, product) would conflate every model in the line into one entry and pool their
    certifications. A row here marks the string as **non-identifying**: the machine is then treated
    as a custom build (identified by its motherboard) and the submitter is asked whether it has a
    real model number the firmware did not report.

    Editable by admins, like the alias tables, because which strings are generic is a judgement that
    grows as new hardware appears. ``vendor`` may be blank to match any manufacturer; matching is
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                Lower("vendor"), Lower("product"), name="unique_generic_model",
            ),
        ]

    def __str__(self) -> str:
        return f"{self.vendor or '(any vendor)'} / {self.product}"


@receiver(post_save, sender=GenericModel)
@receiver(post_delete, sender=GenericModel)
def _invalidate_generic_model_cache(sender, **kwargs) -> None:
    from lumina.results import inventory_extract

    inventory_extract.invalidate_generic_cache()
