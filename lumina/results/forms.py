...
class BundleUploadForm(forms.Form):
    ...
    bundle = forms.FileField(
        label="Result bundle",
        help_text="The .tar.zst produced by `alma-certify bundle` "
    ...
    # The web equivalent of ``--anonymous``. It matters most for a benchmark run, which is
    # pending the moment it lands and so never sees the proposal form where a validation run
    # can still answer this before review.
    publish_anonymously = forms.BooleanField(
        required=False,
        label="List this run anonymously",
        help_text="Show “Anonymous” instead of your username where this is published. "
                  "Reviewers still see who submitted it, and you can change it later.",
    )
...
class ComponentTiesMixin:
    ...
    STATIC_CONTROL_FIELDS = (
        ...
        # Also on the run rather than in the proposal: it governs how this evidence is
        # attributed, not what the machine is.
        "publish_anonymously",
    ...
    # "This part is misidentified, and here is what this machine also called it." The submitter's
    # half of the same job the boxes above do for a reviewer.
    COMPONENT_SUGGEST_PREFIX = "tie_suggest_"
    SUGGEST_OTHER = "other"

    # Whether this form lets its reader rewrite a component's vendor and model outright. True for
    # a reviewer, who decides what the catalog says. False for a submitter: their free text landed
    # in ``component_overrides`` and became the catalog component's name, so a typo minted a
    # catalog entry - and now that reviewers have the naming rules, the correction belongs on
    # their side. A submitter tells us the part is wrong and which of their machine's own values
    # is right, which is the thing only they can do.
    COMPONENT_CORRECTIONS = True
    ...
    def _build_component_fields(self) -> None:
        ...
        self._component_suggestions = []
        for index, entry in enumerate(self.component_rows):
            if not self.COMPONENT_CORRECTIONS:
                self._add_component_suggestion(index, entry)
    ...
    def _add_component_suggestion(self, index: int, entry: dict) -> None:
        """One question per part, asked only if the reader opens it: is this right, and if not,
        which of the values your own machine reported is?

        Values, never paths. Being asked which field holds the real model is being asked to
        understand the payload; being asked "is it this one or this one" is a question about the
        hardware in front of you. The path is carried by index and shown to nobody - it is what
        lets a reviewer turn the answer into a naming rule rather than a one-run correction.

        A part with nothing else to offer gets no control at all. There is no state where somebody
        opens this and finds an empty list: the template sends them to the reviewer notes instead.
        """
        from lumina.results import naming

        kind = entry["kind"]
        if getattr(kind, "value", kind) not in naming.RULED_KINDS:
            # Nothing reads a rule for this kind, so an answer about it would go nowhere.
            entry["candidates"] = []
            return
        # This row is being asked the question, whether or not the machine has anything to offer
        # as an answer. Set here rather than inferred from the kind, because the kind is true of
        # the part and this is true of the seat: only a submitter is asked. Inferring it put the
        # submitter's prompt on the reviewer's page beside their own "Correct it".
        entry["asks_suggestion"] = True
        context = naming.build_context(
            getattr(kind, "value", kind), entry.get("device") or {},
            (self.run.inventory if self.run else None) or {},
        )
        entry["candidates"] = naming.candidates(context, kind, entry["raw_model"])
        if not entry["candidates"]:
            return
        name = self._control(f"{self.COMPONENT_SUGGEST_PREFIX}{index}")
        self.fields[name] = forms.ChoiceField(
            required=False, choices=[
                ("", "Yes, that is right"),
                *((str(n), c["value"]) for n, c in enumerate(entry["candidates"])),
                (self.SUGGEST_OTHER, "Something else"),
            ],
        )
        entry["suggest_field"] = self[name]
        self._component_suggestions.append((entry["key"], name, entry["candidates"]))

    def component_suggestions(self) -> dict:
        """``{tie_key: {"path": ..., "value": ...}}`` for the parts the reader said are wrong.

        Nothing for "that is right" and nothing for "something else": the first is silence and the
        second is a prose answer, which belongs in the notes field that already exists rather than
        in a structured column nobody can act on.
        """
        out = {}
        for key, name, candidates in getattr(self, "_component_suggestions", []):
            chosen = (self.cleaned_data.get(name) or "").strip()
            if not chosen.isdigit():
                # "" is "that is right" and "other" is a prose answer; neither is an index.
                continue
            picked = candidates[int(chosen)]
            out[key] = {"path": picked["path"], "value": picked["value"]}
        return out
...
class RunListingProposalForm(ComponentTiesMixin, forms.Form):
    # A submitter says a part is wrong and which of their machine's own values is right. They do
    # not rewrite it: free text here landed in ``component_overrides`` and became the catalog
    # component's name, so a typo minted a catalog entry, and the correction belongs with the
    # reviewer who now has the naming rules to make it stick across every machine of that shape.
    COMPONENT_CORRECTIONS = False
    ...
    pre_release = forms.BooleanField(
        ...
        # ``reveal-toggle`` folds the publish-date field away until this is ticked, using the
        # CSS-only disclosure the identity override uses (static/css/lumina-shared.css). The class
        # is set here, not defaulted by ``bootstrapify``, because it needs both it and the Bootstrap
        # one; the template renders the date inside a following ``.reveal-fields`` sibling.
    ...
    # Attribution, not secrecy: the run publishes either way, the question is whose name is
    # on it. In the same card because a submitter reads it as one question - what does the
    # public see - and this is the last point before review where the answer is theirs alone.
    # Still changeable afterwards from the run page, which the help text says so nobody
    # treats this checkbox as a decision they cannot revisit.
    publish_anonymously = forms.BooleanField(
        required=False, label="List this run anonymously",
        help_text="The published run shows “Anonymous” instead of your username. Reviewers "
                  "and administrators still see who submitted it. You can change this later "
                  "from the run page.",
    )
    ...
    def __init__(self, *args, subject: str = "system", run=None, user=None,
        ...
        # A detected CPU is tied and named in the "Components this run is evidence for" section
        # below, so top-of-page processor fields would only duplicate it - drop both, and the
        # "Processor" card disappears with them. When nothing was detected there is no component to
        # carry the CPU, so the model box and the family picker stay; a typed model still makes the
        # family a derived value, so the picker is offered only alongside an empty model box.
        if run is not None and (run.cpu_model or "").strip():
            del self.fields["cpu_model"]
    ...
    # Shares the Publication card with the embargo above but not its reveal logic: this one
    # is never folded away, because it applies to every run whether or not anything is held
    # back.
    PUBLICATION_FIELDS = ("publish_anonymously",)
    ...
    def publication_rows(self):
        return self._present(self.PUBLICATION_FIELDS)
    ...
    def _build_category_fields(self, subject: str) -> None:
        ...
        Every machine the propose form describes is catalogued as a System now, a custom build
        included: its board identifies a System, and the tags bind to that System at approval. So
        the form offers the system axes rather than the component ones whatever the subject is.
        """
        from lumina.taxonomy.forms import category_picker_field
        from lumina.taxonomy.models import Category, CategoryValue

        applies = Category.APPLIES_SYSTEM
    ...
    def _vendor_choices(user) -> list:
        ...
        vendors = vendors_for_submission(user, verified_only=True).order_by("name")
        return [("", "- None (submitting as yourself) -")] + [
            (v.slug, v.name) for v in vendors
        ]
    ...
    def initial_from_run(cls, run) -> dict:
        ...
        Keyed on ``effective_system_kind``, not the raw detection: a machine whose firmware reports
        a generic product line ("Super Server") is a custom build identified by its board, so the
        form must prefill the board and preselect "custom build" rather than offering the generic
        system string under a "vendor system" toggle.
        """
        ...
        initial = {
            ...
            # Always one of the two kinds, and the *effective* one so a generic product line
            # preselects "custom build" (it is identified by its board) rather than the "vendor
            # system" its raw detection guessed. There is no third kind: a machine is claimed to be
            # a vendor-built product or it is not.
            "machine_kind": run.effective_system_kind,
        ...
        # Set at ingest from ``--anonymous`` or the account-wide default, so the box arrives
        # already ticked for a submitter who asked for either. Showing it unticked would read
        # as an offer rather than the state it is.
        initial["publish_anonymously"] = run.publish_anonymously
