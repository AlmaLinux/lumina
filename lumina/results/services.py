...
from lumina.results.inventory_extract import identity_key, is_generic_model, is_placeholder
...
def find_matching_system(vendor_name: str, product: str):
    ...
    if not vendor_name or not product:
        return None
    # A generic product line names no specific machine, so it must never name-match a listing:
    # every model in the line would otherwise collapse onto whichever was catalogued first. The
    # explicit alias above still wins, so a human can map a specific unit by hand if they choose.
    if is_generic_model(vendor_name, product):
        return None
...
def existing_listing_for(run: TestRun):
    ...
    if kind == SystemKind.CUSTOM:
        # A custom build's machine listing is the System its board identifies. Prefer that System
        # so a re-validation reads as one and the preview names it; fall back to the board component
        # (before its System exists, or a pre-existing one) so callers still get a listing.
        board = run.listing_components.filter(
            kind=_ComponentKind.motherboard.value
        ).first() or find_matching_board(run.board_vendor, run.board_model)
        if board is not None:
            system = system_for_board(board)
...
def apply_alias_kinds(runs) -> list:
    ...
    if not runs:
        return runs
    lookup = {
        identity_key(vendor, product): kind
    ...
    for run in runs:
        ...
        for vendor, product in run.reported_identity_pairs:
            found = lookup.get(identity_key(vendor, product))
            if found:
...
def sibling_runs(run: TestRun, statuses):
    ...
    **And the same claim, which the identity does not imply.** A host can be the subject of a
    ...
    # "Same hardware" is the run's reported identity, taken from the one place that decides what
    # that is - ``reported_identity_pairs`` - which already drops generic product lines and
    # placeholder strings, so no run keys on something shared by unrelated machines. Each pair is
    # matched against either table, so a board reported in one run's system slot still lines up with
    # the same board in another's.
    identity = Q()
    for vendor, product in run.reported_identity_pairs:
        identity |= (
            Q(system_vendor__iexact=vendor, system_product__iexact=product)
            | Q(board_vendor__iexact=vendor, board_model__iexact=product)
        )
    if not identity:
...
def _majors_proved_by_pending_siblings(run: TestRun) -> set[int]:
    """AlmaLinux majors a passing run of this machine is already queued for.

    A declared major reads as "nobody has proved this" only when that is really so. When a sibling
    run passed on its own major and is waiting in the review queue, approving it - or the "Approve
    all" button - records that run's tier, so the major is backed by evidence, not merely stated.
    The review page lists those siblings just below the proposal, and the declaration wording used
    to flatly contradict them. Passing runs only, exactly as ``approve_group`` sweeps them.
    """
    return {
        sibling.alma_release.major
        for sibling in pending_sibling_runs(run)
        if sibling.alma_release_id is not None and sibling.verdict() is True
    }
    ...
    for component in run.listing_components.all():
        ...
        if level_outranks(level, theirs):
            capped.append({
                "label": str(component),
                "level": theirs,
                "level_display": level_display(theirs),
            })
    return capped
...
def proposal_effect(run: TestRun) -> dict:
    ...
    if run.is_scoped:
        ...
        return {
            ...
            "level_display": level_display(_incoming_level(run, None, "")),
    ...
    # New declarations, annotated with which majors a passing sibling run already sits in the queue
    # for, so the page does not tell a reviewer a release is unproven or dropped while its
    # attestation sits in the queue right below. Applies whether a listing is being created (a bare
    # declaration would otherwise be claimed for it) or already exists (a re-validation would
    # otherwise say the release is dropped) - in both, a queued sibling run adds the real thing on
    # approval.
    new_declarations = _new_declared_releases(run, listing, proposal)
    proved_in_queue = _majors_proved_by_pending_siblings(run)
    for declaration in new_declarations:
        declaration["proved_in_queue"] = declaration["major"] in proved_in_queue
    ...
    return {
        ...
        # The declared majors split by wording: those a queued run already proves (approve it too to
        # record its tier) and those genuinely unbacked (the submitter is stating them).
        "declares_queued": [d for d in new_declarations if d["proved_in_queue"]],
        "declares_unproven": [d for d in new_declarations if not d["proved_in_queue"]],
...
def apply_anonymity_to_runs(user, *, anonymous: bool) -> int:
    """Bring every run this account has submitted into line with one answer.

    The deliberate, opt-in half of the account-wide preference: it reaches published
    runs, and it overwrites per-run choices, which is exactly what somebody asking for
    "all my runs" means and exactly why it is never implied by saving the preference on
    its own. Returns how many runs actually changed, so the caller can say.

    One row per changed run in the audit log rather than a single summary entry: the
    question later is always "why does *this* run say Anonymous", and it is asked on the
    run, not on the account.
    """
    changed = list(
        TestRun.objects.filter(submitter=user).exclude(publish_anonymously=anonymous)
    )
    if not changed:
        return 0
    TestRun.objects.filter(pk__in=[r.pk for r in changed]).update(
        publish_anonymously=anonymous
    )
    for run in changed:
        log_action("test_run.set_anonymity", target=run, actor=user,
                   after={"publish_anonymously": anonymous, "via": "account_settings"})
    return len(changed)


def set_run_anonymity(run: TestRun, *, by, anonymous: bool) -> TestRun:
    """Change whether a run is published under its submitter's name.

    Allowed at any status, published runs included: that is the whole point. Somebody
    who contributed evidence and later decided they would rather not be named, or who
    published anonymously and now wants the credit, should be able to say so without
    asking a reviewer - nothing about the evidence changes either way, only the label
    the public reads.

    Logged like every other change of state, because "why does this run say Anonymous"
    is a question a reviewer will eventually ask, and the submitter is still recorded on
    the run itself regardless.
    """
    if run.submitter_id != getattr(by, "pk", None):
        raise ReviewError("Only the submitter can change how their own run is attributed.")
    if run.publish_anonymously == anonymous:
        return run
    run.publish_anonymously = anonymous
    run.save(update_fields=["publish_anonymously"])
    log_action("test_run.set_anonymity", target=run, actor=by,
               after={"publish_anonymously": anonymous})
    return run
...
def cpu_brand(run: TestRun) -> str:
    """The catalog manufacturer for a run's CPU: its reported CPUID vendor mapped to a real brand
    ("GenuineIntel" -> "Intel"), falling back to the reported string, then "Unknown". Shared by
    ``component_tie_targets`` (which records the tie) and ``ensure_component_ties`` (which creates the
    CPU component) so the two cannot name the same silicon differently.

    The table moved to ``vendors.cpu_aliases`` so the published hardware survey reads the
    same one: the catalog saying "Intel" while the statistics said "GenuineIntel" for the
...
def tieable_nics(run: TestRun) -> list[dict]:
    ...
    tieable = []
    # Categorized server-side from the raw PCI enumeration, so a NIC the collector's netdev view
    # missed is still seen; the driver here is the bound kernel driver lspci reports.
    for nic in categorized_devices(run)["nics"]:
...
def tieable_gpus(run: TestRun) -> list[dict]:
    ...
    # Categorized server-side (class 03) from the raw PCI enumeration, same path as NICs.
    #
    # Named by ``gpu_display_name``, which is the same call the census and the survey's own
    # columns make. It used to apply the AMD APU rule here, inline, while the statistics applied
    # nothing and the ingest column applied something else again - so one machine was "Radeon
    # 780M" on its run page and "HawkPoint1" in the published figures. One function, and the three
    # cannot disagree.
    #
    # The rules it applies: an AMD die codename is replaced by the product name the CPU brand
    # string advertises, and a name that identifies nothing ("Intel Graphics", which pci.ids gives
    # every recent Intel iGPU) is qualified with the CPU it is integrated into.
    devices = [gpu for gpu in categorized_devices(run)["gpus"] if isinstance(gpu, dict)]
    cpu_model = run.cpu_model or ""
    resolved = []
    for gpu in devices:
        vendor, raw = gpu_identity(gpu)
        if not (vendor and raw and gpu.get("driver")):
            continue
        # The status, not just the name: a reviewer looking at this row needs to know whether a
        # rule produced the name, whether lumina's built-in behaviour did, or whether nothing did
        # and the part is still called something that identifies nothing.
        status = gpu_name_status(gpu, cpu_model, devices)
        model = status["name"]
        reported = normalize_gpu_model(raw)
        # Both: ``model`` is what the part is catalogued as, ``reported_model`` what the machine
        # said. The preview shows one against the other, so a reader can see that approving this
        # run files "Intel Graphics" under the name of the CPU it is integrated into.
        # ``raw`` and not ``reported``: what the machine said, before any normalization. That is
        # the half of the translation the preview exists to show ("CometLake-S GT2 [UHD Graphics
        # 630]" becomes "UHD Graphics 630"), and the normalized form would already be the answer.
        entry = {**gpu, "vendor": vendor, "model": model, "reported_model": raw,
                 "naming": status}
        if model != reported and not is_generic_gpu_name(reported):
            # The codename is kept: it identifies the silicon generation in a way the marketing
            # name does not. Not for a generic name, where what was reported says nothing worth
            # keeping a field for.
            entry["asic"] = reported
...
def rule_excluded_ties(run: TestRun) -> dict:
    """``{tie_key: reason}`` for the run's tieable devices an exclusion rule matches.

    Computed over the same tieable devices ``component_tie_targets`` emits and keyed the same way
    (``tie_key`` on the resolved model), so a seeded exclusion lines up exactly with the row it is
    about - a curated ``ComponentExclusionRule`` (an onboard iGPU) or the categorical rule (a BMC
    display adapter, from a vendor that makes no accelerators).
    """
    ...
    for kind, devices in (
        (ComponentKind.gpu, tieable_gpus(run)),
        (ComponentKind.nic, tieable_nics(run)),
    ):
        for device in devices:
            reason = exclusion_reason(device, kind, rules=rules)
            if reason:
                reasons[tie_key(kind, device["model"])] = reason
    return reasons


def seed_rule_exclusions(run: TestRun) -> None:
    """At ingest, untick by default the devices an exclusion rule matches, recording why.

    Seeds ``excluded_component_ties`` so the review screen shows them unticked from the start, and
    ``component_exclusion_reasons`` so it can say which rule. Runs before any human review, so it
    never overrides a reviewer's decision - and a reviewer can tick any of them to include it, which
    removes it from the excluded set and sticks.
    """
    reasons = rule_excluded_ties(run)
    if not reasons:
        return
    run.excluded_component_ties = list(
        dict.fromkeys(list(run.excluded_component_ties or []) + list(reasons))
    )
    run.component_exclusion_reasons = reasons
    run.save(update_fields=["excluded_component_ties", "component_exclusion_reasons"])


def rule_excluded_summary(run: TestRun) -> list[dict]:
    """``[{kind, component, reason}]`` for the devices a rule unticked, for the submitter's notice.

    Read off ``preview_component_ties`` so it names each part the way the review screen does, and
    reflects a reviewer's later include (the row stops being excluded, so it drops out here too).
    """
    return [
        {"kind": entry["kind_label"], "component": entry["raw_model"],
...
def component_tie_targets(run: TestRun) -> list[dict]:
    ...
    *report* said, before any override is applied. It has to be: the key is what an exclusion
    ...
    """
    from lumina.results import naming as naming_rules
    from lumina.results.component_match import name_status

    overrides = run.component_overrides or {}

    def entry(kind, brand: str, raw_model: str, attributes: dict, device: dict = None,
              reported_model: str | None = None, naming: dict | None = None) -> dict:
        key = tie_key(kind, raw_model)
        chosen = overrides.get(key) or {}
        # A naming rule's answer, applied after the key is taken so the key stays on what the
        # report said. Deriving the key from a resolved name would move the very row somebody had
        # filed a decision against - an exclusion, a declined vendor claim, a brand fix - which is
        # the bug the CPU entry below already carries a paragraph about.
        #
        # Under a human correction, never over it. Somebody holding the machine who typed what the
        # part is outranks a pattern written about machines in general.
        ruled = (naming or {}).get("name") or ""
        # PCI ids for the parts that have them (GPU/NIC), so the review screen can offer to blacklist
        # this exact model - vendor:device is what ComponentExclusionRule matches on. Blank for a
        # CPU or motherboard, which are not PCI devices and carry no such rule.
        vid = pci_vendor_id(device) if device else ""
        did = pci_device_id(device) if device else ""
        return {
            "key": key,
            "kind": kind,
            # What the report said, kept so the form can show the correction against it.
            "reported_brand": brand,
            # What the machine said, which for a GPU is not the string beside it: the name is
            # resolved before it gets here, so the row would otherwise show the translation
            # against itself and the reader would never see that one happened.
            "reported_model": reported_model or raw_model,
            "brand": (chosen.get("brand") or "").strip() or brand,
            "raw_model": (chosen.get("model") or "").strip() or ruled or raw_model,
            "overridden": bool(
            ...
            "pci_vendor_id": vid,
            "pci_device_id": did,
            # Why this part is shown under this name. GPUs carry it on the device, having been
            # resolved before they reach here; a CPU and a motherboard are resolved at the call.
            "naming": naming or (device or {}).get("naming"),
            # What identifies this part, for a caller that wants to ask the rules about it: the
            # PCI device for a card, the two reported strings for a CPU or a board. Carried rather
            # than recomputed so the one place that knows how a kind is identified stays one.
            "device": device if device is not None else {"vendor": brand, "model": raw_model},
            # Whether a naming rule would reach this kind at all. The offer to write one is worth
            # making only where something reads it.
            "ruled": kind.value in naming_rules.RULED_KINDS,
            # Enough to pre-fill a precise exclusion rule (vendor + device); a device missing either
            # is left to the admin rather than seeding a too-broad rule from the review screen.
            "blacklistable": bool(vid and did),
        }
    ...
    if run.board_vendor and run.board_model:
        targets.append(entry(
            ...
            naming=name_status(
                ComponentKind.motherboard, run.board_vendor, run.board_model, payload),
        ))
    ...
    if run.cpu_model:
        targets.append(entry(
            ...
            run.cpu_model,
            {},
            naming=name_status(ComponentKind.cpu, cpu_brand(run), run.cpu_model, payload),
        ))

    for gpu in tieable_gpus(run):
        # The resolved name, which is what the component is created under. ``reported_model``
        # rides along on the device dict so the row can show what the machine actually said
        # against what approving this would file it as.
        targets.append(entry(
            ComponentKind.gpu,
            gpu["vendor"],
            gpu["model"],
            ...
            device=gpu,
            reported_model=gpu.get("reported_model"),
        ))
...
def preview_component_ties(run: TestRun) -> list[dict]:
    ...
    exclusion_reasons = run.component_exclusion_reasons or {}
    preview: list[dict] = []
    for target in component_tie_targets(run):
        vendor = resolve_vendor(target["brand"])
        family = None
        ...
        preview.append({
            ...
            # Set only when a rule is why it is unticked, so the screen can say "excluded per
            # rule: <reason>" and offer to include it anyway. Absent for a plain human untick.
            "excluded_reason": exclusion_reasons.get(key) if key in excluded else None,
            "kind": target["kind"].value,
            "kind_label": target["kind"].label,
            "brand": vendor.name if vendor else target["brand"],
            "raw_model": target["raw_model"],
            # PCI ids and a flag, forwarded so the review screen can offer to blacklist this exact
            # model (a permanent ComponentExclusionRule keyed on vendor:device). Blank/False for a
            # CPU or motherboard, which are not PCI devices.
            "pci_vendor_id": target["pci_vendor_id"],
            "pci_device_id": target["pci_device_id"],
            "blacklistable": target["blacklistable"],
            # Why this part is shown under this name, and whether a rule could say otherwise.
            # Forwarded so the row can say which, since a renamed part and an untouched one look
            # the same once resolved.
            "naming": target["naming"],
            "ruled": target["ruled"],
            # What identifies the part, so a form can ask the naming rules about it without
            # knowing how each kind is identified.
            "device": target["device"],
            # What the submitter said this part really is, if they said anything. Shown to the
            # reviewer beside the part rather than buried in a notes field, because the path it
            # came from is what turns it into a rule.
            "suggestion": (run.component_suggestions or {}).get(target["key"]),
...
def system_for_board(board):
    """The System a custom build's motherboard identifies, or None.

    A board's identity maps one-to-one to a System of the same vendor and name, so the resolver
    (``existing_listing_for``) and the creator (``_ensure_custom_system``) agree on which System is
    the machine for a board-identified build.
    """
    return System.objects.filter(vendor=board.vendor, name__iexact=board.name).first()


def _ensure_custom_system(run: TestRun) -> None:
    """Give a custom build a System listing identified by its motherboard.

    The chosen model for self-builds: one System per board (vendor = board maker, name = board
    model), so every build on that board aggregates into a single machine listing shown under
    Systems, with the board, CPU and GPUs tied on as components exactly as a prebuilt's are. The
    System mirrors the board component's identity one-to-one, so it dedups on precisely what the
    board dedups on - a second build on the same board resolves this same System.

    A no-op unless this is an unscoped custom build with no System yet: a prebuilt already has one
    (``create_listings_from_run`` set it), and a scoped run never gets one.
    """
    if run.listing_system_id or run.is_scoped:
        return
    if run.effective_system_kind != SystemKind.CUSTOM:
        return
    board = (
        run.listing_components.filter(kind=ComponentKind.motherboard.value).first()
        or find_matching_board(run.board_vendor, run.board_model)
    )
    if board is None:
        return
    system = system_for_board(board)
    if system is None:
        system = System.objects.create(
            ...
            vendor_spec_url=board.vendor_spec_url,
            created_by=run.submitter,
            owner_vendor=board.owner_vendor,
        )
    run.listing_system = system
    run.save(update_fields=["listing_system"])
...
def ensure_component_ties(run: TestRun) -> None:
    ...
    # A custom build's System is its motherboard promoted to a machine listing. Resolve or create
    # it here (from the board) so its parts tie to it below and it is attested, the same end state a
    # prebuilt reaches. This covers a repeat run whose board auto-linked at ingest, which skips
    # create_listings_from_run. A no-op for prebuilt (already has a System) and scoped runs.
    _ensure_custom_system(run)
    ...
    for target in component_tie_targets(run):
        ...
        if run.listing_system_id:
            if kind == ComponentKind.cpu:
                ...
                # The board and GPUs attach as related components of the machine's System: a
                # prebuilt model, or, for a custom build, the System its own board identifies.
...
def submit_for_review(run: TestRun, *, by) -> TestRun:
    ...
    emit("run.submitted", target=run, actor=by)
    return run


@transaction.atomic
def apply_run_certification(run: TestRun, *, tie: bool = True) -> bool:
    """Fold an approved, releasable run's evidence into the catalog, and return whether it produced a
    new attestation.

    One ordered sequence, in one place, because the order matters and three callers need it
    identically (``approve_run``, the assign-listing edit, and ``publish_due_runs``). Components are
    linked first; then ``record_compatibility`` creates the AlmaLinux release row *before*
    ``_apply_attestation`` runs, because an attestation now hangs off that row - reversed, a
    listing's first-ever run would attest to nothing at all. The callers own the gate (embargo,
    status); this owns the coupling.

    ``tie=False`` repairs an approval that already happened: it records what the run's existing
    ties should have earned and creates no new ones. Resolving the parts again would be wrong
    there, because a naming rule or an exclusion written since would resolve them differently and
    a repair would quietly file the machine's hardware under new names.
    """
    if tie:
        ensure_component_ties(run)
    record_compatibility(run)
    record_architecture(run)
    return _apply_attestation(run)


def _accept_survey_forks(run, *, by) -> None:
    """Mark the census records forked from this run as reviewed.

    Deferred import, and never allowed to raise: the survey is a separate stream and a
    problem there must not undo a certification decision a reviewer has already made.
    """
    from lumina.survey.services import accept_forks_for_run

    accept_forks_for_run(run, by=by)
...
def approve_run(run: TestRun, *, by, notes: str = "") -> TestRun:
    ...
    # Withheld only while unreleased hardware is still under a request-embargo. The rule (blank date
    # holds until lifted, a past date publishes now) lives on the property, so approval and
    # assign_listing cannot disagree about it.
    embargoed = run.is_embargoed_by_request
    ...
    if run.is_embargoed:
        ...
        attested = apply_run_certification(run)
    log_action(
        "test_run.approve",
        target=run,
        actor=by,
    ...
    emit("run.approved", target=run, actor=by)
    # The run's survey fork is reviewed with this decision rather than moderated on its own.
    _accept_survey_forks(run, by=by)
    return run
...
def reject_run(run: TestRun, *, by, reason: str = "") -> TestRun:
    ...
    # there.
    if run.status == TestRun.STATUS_QUARANTINED:
        stamp_review_decision(run, status=TestRun.STATUS_REJECTED, by=by, reason=reason)
        ...
        emit("run.rejected", target=run, actor=by)
        return run
    ...
    log_action("test_run.reject", target=run, actor=by,
               after={"status": run.status}, notes=reason)
    emit("run.rejected", target=run, actor=by)
    # Deliberately does not touch the run's survey fork. The census asks what hardware
    # exists and runs AlmaLinux, not what passed: a run rejected for failing a test, or for
    # wrong listing details, still came from a real machine. Removing it would make the
    # published counts a survey of successful certifications instead.
    return run
...
def request_run_changes(run: TestRun, *, by, reason: str = "") -> TestRun:
    ...
    stamp_review_decision(run, status=TestRun.STATUS_NEEDS_CHANGES, by=by, reason=reason)
    log_action("test_run.request_changes", target=run, actor=by,
               after={"status": run.status}, notes=reason)
    emit("run.needs_changes", target=run, actor=by)
    return run


def waive_result(result: TestResult, *, by, reason: str) -> TestResult:
    """Waive one failing result so its run may certify in spite of it.

    A reviewer's judgment that this failure does not condemn the machine - a known-benign error, or a
    check that does not apply to this hardware - recorded on the result with who and why, because
    certifying over a failure is an attributable decision. Only a gating failure can be waived: a
    pass, a skip, or a non-gating result (informational, benchmark) does not block certification, so
    waiving it would mean nothing and is refused rather than logged as though it did.
    """
    if not (reason or "").strip():
        raise ReviewError("A waiver needs a reason.")
    if not (
        result.severity in GATING_SEVERITIES
        and result.status in (ResultStatus.FAIL, ResultStatus.ERROR)
    ):
        raise ReviewError(
            f"Only a failing validation can be waived; {result.test_id} does not block certification."
        )
    result.waived_by = by
    result.waived_at = timezone.now()
    result.waiver_reason = reason.strip()
    result.save(update_fields=["waived_by", "waived_at", "waiver_reason"])
    log_action(
        "test_result.waive", target=result.run, actor=by,
        after={"test_id": result.test_id, "reason": result.waiver_reason},
    )
    return result


def unwaive_result(result: TestResult, *, by) -> TestResult:
    """Lift a waiver, so the failure blocks certification again."""
    if not result.is_waived:
        raise ReviewError(f"{result.test_id} is not waived.")
    result.waived_by = None
    result.waived_at = None
    result.waiver_reason = ""
    result.save(update_fields=["waived_by", "waived_at", "waiver_reason"])
    log_action("test_result.unwaive", target=result.run, actor=by,
               after={"test_id": result.test_id})
    return result
...
def create_listings_from_run(run: TestRun, *, by) -> list:
    ...
    if effective_kind == SystemKind.PREBUILT:
        ...
        # machine's own details (description via the System's creation, plus categories and declared
        # support here) bind to that System rather than to the shared board part.
        _ensure_custom_system(run)
        if run.listing_system_id:
            apply_proposal_metadata(run, run.listing_system)
    else:
        raise ReviewError(
...
def reassessable_as(run: TestRun, user):
    """The verified vendor ``user`` may re-assess ``run`` under, or ``None``.

    The self-service half of the tier story the attestation writer already anticipates (see
    ``record_attestation``: "a repeat carrying a higher tier upgrades their existing statement").
    Someone validates a machine as a community member, later joins - and has a reviewer verify - the
    vendor that owns the listing, and can then have their *own* evidence count at the vendor tier
    without re-running the suite.

    Offered only for the user's own run whose certification is already applied (an attestation
    exists), whose listing a *verified* vendor they submit for owns, and which is not already
    attributed to that vendor - so the button never appears when it would change nothing.
    """
    if not getattr(user, "is_authenticated", False) or run.submitter_id != user.pk:
        return None
    # An attestation exists only once certification has actually been applied; this excludes an
    # unapproved run and an approved-but-embargoed one, whose tier is not the reviewer's to raise
    # early through here.
    if not run.attestations.exists():
        return None
    from lumina.vendors.services import is_submit_member

    for listing in scoped_listings(run):
        vendor = getattr(listing, "owner_vendor", None)
        if (vendor and vendor.verified and run.on_behalf_of_id != vendor.pk
                and is_submit_member(user, vendor.pk)):
            return vendor
    return None


@transaction.atomic
def reassess_run_as_vendor(run: TestRun, *, vendor, by) -> bool:
    """Re-attribute ``run`` to ``vendor`` and re-apply its attestation, upgrading the tier.

    Re-checks ``reassessable_as`` - the same rule the button renders on - so a POST cannot upgrade
    someone else's run or name a vendor the user may not. All this changes is the run's attribution:
    ``_apply_attestation`` then re-derives the tier (now the vendor's, per ``effective_level``) and
    ``record_attestation`` upgrades the person's existing statement rather than adding a second.
    Returns whether anything was newly attested (an upgrade of an existing row returns ``False``).
    """
    eligible = reassessable_as(run, by)
    if eligible is None or eligible.pk != vendor.pk:
        raise PermissionError("Not eligible to re-assess this run as that vendor.")
    run.on_behalf_of = vendor
    run.save(update_fields=["on_behalf_of"])
    created = _apply_attestation(run)
    log_action(
        "test_run.reassessed", target=run, actor=by,
        after={"vendor": vendor.name, "vendor_id": vendor.pk},
    )
    return created
...
def _attest_one(run: TestRun, listing) -> bool:
    ...
    created = record_attestation(
        version=version, listing=listing, attested_by=run.submitter, level=level, source=run,
    )
...
def release_held_run(run: TestRun) -> bool:
    ...
    log_action("test_run.publish", target=run,
               after={"published_at": now.isoformat(),
                      "attestation_created": attested})
    # No actor: a hold most often ends because a date arrived, and naming the reviewer who cleared
    # it in the one case where there is one would make the other case look like it lost something.
    emit("run.published", target=run)
    return True
