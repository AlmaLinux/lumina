...
# Keyed on the suite's benchmark ids. An id with no entry falls back to a
# derived label, so a benchmark added to the suite before this map is updated
# reads as "Fio 4k randread" rather than breaking the page.
BENCHMARK_LABELS = {
    "bench.cpu.sysbench-single": "CPU, single core",
    "bench.cpu.sysbench-multi": "CPU, all cores",
    "bench.cpu.stressng-matrix": "CPU matrix math",
    "bench.mem.bandwidth": "Memory bandwidth",
    "bench.mem.latency": "Memory latency",
    "bench.mem.stressng-stream": "Memory stream",
    # Retired from the suite, kept here because published results still carry
    # these ids and a stored row deserves its name.
    "bench.storage.fio-4k-randread": "Disk random read",
    "bench.storage.fio-4k-randwrite": "Disk random write",
    "bench.storage.fio-seq-read": "Disk sequential read",
    "bench.storage.fio-seq-write": "Disk sequential write",
    "bench.net.iperf3-tcp": "Network throughput",
    "bench.net.iperf3-reverse": "Network throughput, inbound",
    "bench.crypto.openssl-aes256gcm": "AES-256-GCM",
    "bench.crypto.openssl-sha256": "SHA-256",
    "bench.crypto.openssl-rsa4096": "RSA-4096 signing",
    "bench.compress.zstd": "zstd compression",
    "bench.compress.xz": "xz compression",
    "bench.compile.python": "Python build",
    "bench.sched.hackbench": "Scheduler, hackbench",
    "bench.sched.stressng-switch": "Context switching",
    "bench.gpu.clpeak": "GPU compute",
    "bench.gpu.cuda-bandwidth": "GPU transfer",
}
...
def benchmark_label(benchmark_id: str) -> str:
    """A human name for a benchmark id, derived if it is not in the map."""
    known = BENCHMARK_LABELS.get(benchmark_id)
    if known:
        return known
