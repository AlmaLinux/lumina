...
def test_the_name_carried_in_a_cpu_brand_string(cpu_model, expected):
    """Asserted against the rule that ships rather than a copy of its pattern. The extraction used
    to be a module-level regex and two derived context fields, which meant the one AMD phrasing
    somebody had anticipated was the only one that could ever work and widening it took a release.
    It is a row now, and this is that row doing the work."""
    import re

    from lumina.hardware.models import ComponentNamingRule
    from lumina.results import naming

    rule = ComponentNamingRule.objects.get(built_in=True, priority=10)
    context = naming.build_context("gpu", {}, {"summary": {"cpus": [{"model": cpu_model}]}})
    # The rule's pattern against the rule's field, then the rule's template over the captures,
    # which is what the engine does. Its other conditions are about the card rather than the CPU
    # string, and this is the CPU string's half.
    found = re.search(rule.model_pattern, naming.lookup(context, rule.match_field) or "", re.I)

    assert naming.render(rule.build, context, found) == expected
...
def test_a_named_amd_card_keeps_its_own_name(submitter):
    ...
    assert _gpus(run) == {"c1:00.0": "Radeon RX 7600"}
...
def test_non_amd_gpus_are_untouched(submitter):
    ...
    assert _gpus(run) == {"01:00.0": "GeForce RTX 4060"}
...
# --- tying a benchmark row to a PCI card ----------------------------------------
#
# clpeak names a device per backend and its result carries no PCI id, so one physical card is named
# differently by Vulkan and OpenCL. pci_names ties a benchmark device back to the run's inventory
# GPU - which does carry the ids - to a "vendor:device" key, so the card's backends group as one.


def test_pci_device_id_reads_the_trailing_id():
    from lumina.results.pci_names import pci_device_id

    assert pci_device_id({"pci_ids": {"device": "AD102GL [L40S] [26b9]"}}) == "26b9"
    assert pci_device_id({"pci_ids": {"device": "Arrow Lake-U [Intel Graphics] [7d55]"}}) == "7d55"
    assert pci_device_id({"pci_ids": {"device": "no id here"}}) == ""
    assert pci_device_id({}) == ""


_INTEL = {"pci_ids": {"vendor": "Intel Corporation [8086]",
                      "device": "Arrow Lake-U [Intel Graphics] [7d55]"}, "driver": "i915"}
_NVIDIA = {"pci_ids": {"vendor": "NVIDIA Corporation [10de]",
                       "device": "AD102GL [L40S] [26b9]"}, "driver": "nvidia"}


def test_a_cards_backends_resolve_to_one_pci_key():
    from lumina.results.pci_names import benchmark_gpu_pci_id

    # One Intel iGPU: both the Vulkan and the OpenCL name resolve to it, whatever they were called.
    assert benchmark_gpu_pci_id("Intel Graphics (ARL)", [_INTEL]) == "8086:7d55"
    assert benchmark_gpu_pci_id("Intel Graphics", [_INTEL]) == "8086:7d55"


def test_the_right_vendor_is_picked_in_a_mixed_system():
    from lumina.results.pci_names import benchmark_gpu_pci_id

    gpus = [_INTEL, _NVIDIA]
    assert benchmark_gpu_pci_id("Intel Graphics (ARL)", gpus) == "8086:7d55"
    assert benchmark_gpu_pci_id("NVIDIA L40S", gpus) == "10de:26b9"


def test_no_key_when_it_cannot_be_pinned_down():
    from lumina.results.pci_names import benchmark_gpu_pci_id

    two_intel = [
        _INTEL,
        {"pci_ids": {"vendor": "Intel Corporation [8086]",
                     "device": "Raptor Lake-P [Iris Xe] [a7a0]"}, "driver": "i915"},
    ]
    # Two different Intel GPUs: which one an Intel-named device is cannot be told, so no key.
    assert benchmark_gpu_pci_id("Intel Graphics", two_intel) == ""
    # A device whose vendor is not in the inventory is not tied to some other vendor's card.
    assert benchmark_gpu_pci_id("NVIDIA L40S", [_INTEL]) == ""
    # No GPUs at all.
    assert benchmark_gpu_pci_id("Intel Graphics", []) == ""


# --- NIC categorization is server-side too --------------------------------------
#
# The reported bug: a Mellanox with mlx5_core bound did not appear as a NIC, because the collector
# categorized NICs from kernel netdevs. Now the collector reports raw pci_devices and lumina
# categorizes (class 02), so the card is a tieable NIC whether or not a netdev was enumerated.


def test_a_nic_in_pci_devices_is_tieable_without_a_netdev(submitter):
    inventory = f.default_inventory()
    inventory["summary"]["nics"] = []  # collector's netdev view saw nothing for it
    inventory["summary"]["pci_devices"] = [{
    ...
    nics = services.tieable_nics(run)
    assert [n["model"] for n in nics] == ["MT27710 Family [ConnectX-4 Lx]"]
    assert nics[0]["vendor"] == "Mellanox Technologies"


# --- one namer for the catalog too ------------------------------------------------


def test_the_catalog_names_a_generic_igpu_by_its_cpu(submitter):
    """``tieable_gpus`` applied the AMD rule inline and nothing else, so an Intel iGPU became a
    catalog component called "Intel Graphics" - a name shared by every recent Intel part. It goes
    through ``gpu_display_name`` now, which is the call the census and the survey columns make."""
    from lumina.results.services import tieable_gpus
    ...
    assert [gpu["model"] for gpu in tieable_gpus(run)] == [
        "Intel Core Ultra 9 275HX (Intel Graphics)"]


def test_the_catalog_keeps_what_the_machine_reported_beside_the_name(submitter):
    """The preview shows one against the other, so a reader can see that approving a run files
    the part under a name the machine never said."""
    from lumina.results.services import tieable_gpus
    ...
    gpu = tieable_gpus(run)[0]

    assert gpu["reported_model"] == "Arrow Lake-S [Intel Graphics]"
    assert gpu["model"] != gpu["reported_model"]


def test_a_generic_igpu_records_no_asic(submitter):
    """``asic`` keeps the die codename a marketing name replaced, which is worth a field. What a
    generic name replaced is "Intel Graphics", which is worth nothing and would read as though the
    silicon were called that."""
    from lumina.results.services import tieable_gpus
    ...
    assert "asic" not in tieable_gpus(run)[0]


def test_the_catalog_and_the_census_name_the_same_card_identically(submitter):
    """The whole point of the shared namer. Asserted against each other rather than a literal."""
    from lumina.results.services import tieable_gpus
    from lumina.survey import devices

    gpus = [{"pci": "00:02.0", "driver": "i915",
             "pci_ids": {"vendor": "Intel Corporation [8086]",
                         "device": "Arrow Lake-S [Intel Graphics]"}}]
    cpu = "Intel(R) Core(TM) Ultra 9 275HX"
    run = _run(submitter, cpu_model=cpu, gpus=gpus)

    catalog = [gpu["model"] for gpu in tieable_gpus(run)]
    census = [model for _vendor, model in devices.countable_gpus(
        {"summary": {"cpus": [{"model": cpu}], "gpus": gpus}})]
