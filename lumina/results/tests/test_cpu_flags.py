...
def test_a_run_without_the_informational_result_still_lists_flags(submitter):
    """A run with no informational result, or one from a suite older than the test.
    The display degrades to the plain list rather than disappearing."""
    run = _run(submitter, run_types=["validate"], results=[])
