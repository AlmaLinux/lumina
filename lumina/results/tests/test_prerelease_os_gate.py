...
def test_the_run_metadata_carries_it(submitter):
    """``--support-from-minor 3`` at ``alma-certify run`` time rides inside the report."""
    run = ingest.ingest_bundle(
...
def test_the_submit_field_overrides_the_metadata(submitter):
    """So ``alma-certify submit --support-from-minor`` can correct a run started without it. Same
