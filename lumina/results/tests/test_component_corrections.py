"""Correcting what a report says a component is.
...
**Half of that was later reversed, deliberately.** The submitter's boxes wrote straight into
``component_overrides``, which names the catalog component, so a typo minted a catalog entry - and
once reviewers had naming rules, a correction made there fixes every machine of that shape rather
than one run. A submitter now says *that* a part is wrong and picks which of their own machine's
reported values is right; see ``test_component_suggestions``. The boxes below are the reviewer's,
and every test that was about the mechanism rather than the seat moved to their form.
...
def _fields(run, _who, kind):
    """The (brand, model) field names for one component on the reviewer's form."""
...
def _row(run, user, kind):
    """The reviewer's form: they are the seat that rewrites a part's vendor and model."""
    form = RunComponentTiesForm(run=run)
...
def test_the_boxes_are_collapsed_on_the_page(client, submitter, reviewer):
...
def test_leaving_a_matched_part_alone_records_nothing(client, submitter, reviewer):
...
def test_ticking_the_override_records_the_correction(client, submitter, reviewer):
    run = _run(submitter)
...
def test_the_reviewer_gets_a_box_per_component(submitter):
    run = _run(submitter)
...
def test_the_submitter_gets_no_boxes_at_all(submitter):
    """The reversal. Their free text became the catalog component's name, so a typo minted a
    catalog entry - and a reviewer's correction now fixes every machine of that shape."""
    run = _run(submitter)

    form = RunListingProposalForm(run=run, user=submitter)

    assert form.component_rows, "the rows are still there to tick and untick"
    for row in form.component_rows:
        assert row.get("brand_field") is None
        assert row.get("model_field") is None
...
def test_posting_a_correction_stores_only_what_changed(client, submitter, reviewer):
...
def test_the_submitters_controls_do_not_pollute_the_listing_proposal(client, submitter):
    run = _run(submitter)
...
def test_the_review_page_renders_the_boxes(client, submitter, reviewer):
    run = _run(submitter)
    client.force_login(reviewer)
...
def test_the_submitters_page_renders_none(client, submitter):
    run = _run(submitter)
    client.force_login(submitter)

    body = client.get(reverse("results:propose_listing", args=[run.uuid])).content.decode()

    assert "tie_brand_" not in body and "tie_model_" not in body
...
def test_the_preview_names_what_a_new_entry_would_be_called(submitter):
    ...
    # ``reported_model``, not ``raw_model``: the name is resolved before it reaches the preview
    # now, so ``raw_model`` is already the catalog's spelling and comparing it against itself
    # would show a translation that had happened somewhere the reader cannot see.
    assert gpu["reported_model"] != gpu["catalog_name"], "nothing to show if they match"
    assert "[" in gpu["reported_model"], "the bracketed string the machine actually reported"
...
def test_a_real_correction_is_still_recorded(client, submitter, reviewer):
    run = _run(submitter)
    form = RunComponentTiesForm(run=run)
