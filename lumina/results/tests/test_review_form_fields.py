...
def test_identity_fields_are_comboboxes(submitter):
    ...
    for field in ("vendor_name", "name", "model_number", "cpu_model"):
...
def test_the_form_is_grouped_into_sections(client, submitter):
    ...
    for heading in ("Identity", "Processor", "AlmaLinux compatibility",
...
def test_a_corrected_kind_changes_what_gets_created(submitter, reviewer):
    """Detected prebuilt, submitter says custom: the catalog must list the
    board, not a system nobody sells."""
    ...
    assert not System.objects.filter(name="B650M PG Riptide").exists()
