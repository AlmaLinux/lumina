...
def test_save_and_submit_does_both_in_one_step(client, submitter):
    """Finishing this form is usually the last thing owed before review, so the page offers to
    go on and submit rather than sending the submitter back to the run to press it there."""
    run = _validate_run(submitter)
    client.force_login(submitter)
    resp = client.post(
        reverse("results:propose_listing", args=[run.uuid]),
    ...
    assert resp.status_code == 302
    assert resp.url == run.get_absolute_url()
    run.refresh_from_db()
    assert run.listing_proposal["model_number"] == "R760XS"
    assert run.status == TestRun.STATUS_PENDING


def test_plain_save_still_leaves_the_run_a_draft(client, submitter):
    """The two buttons are different actions. Saving answers is not releasing the run, and a
    submitter filling the form in over two sittings must not have the first one submit it."""
    run = _validate_run(submitter)
    client.force_login(submitter)
    client.post(
        reverse("results:propose_listing", args=[run.uuid]),
...
def test_a_refused_submission_keeps_the_details_and_returns_to_the_form(
    client, submitter, monkeypatch
):
    """The save happens before the submission is attempted, so a run that cannot go to the queue
    yet does not cost the submitter what they just typed. Back to this form rather than the run
    page, because everything a submission can still be missing is a field of this one."""
    def refuse(run, *, by):
        raise services.ReviewError("Still needed before review: something.")

    monkeypatch.setattr(services, "submit_for_review", refuse)
    run = _validate_run(submitter)
    client.force_login(submitter)
    resp = client.post(
        reverse("results:propose_listing", args=[run.uuid]),
    ...
    assert resp.status_code == 302
    assert resp.url == reverse("results:propose_listing", args=[run.uuid])
    run.refresh_from_db()
    assert run.listing_proposal["name"] == "PowerEdge R760"
    assert run.status == TestRun.STATUS_DRAFT
