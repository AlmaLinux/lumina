...
def test_the_index_lists_only_registry_benchmarks(submitter, reviewer):
    """Lumina decides what is listed, not what is submitted. A benchmark id the registry does not
    define - a retired one, or one it was never taught - is not listed even with a public result
    behind it, and a defined benchmark is listed even before anything has run it."""
    _bench_run(submitter, reviewer, cpu="EPYC 9354", value=1,
               run_id="33333333-0000-0000-0000-000000000001",
               benchmark="bench.sched.hackbench", unit="s", direction="lower_is_better")
    _bench_run(submitter, reviewer, cpu="EPYC 9354", value=2,
               run_id="33333333-0000-0000-0000-000000000002",
               benchmark="bench.made.up", unit="x")

    body = client_get_index()

    assert "bench.cpu.sysbench-multi" in body, "a defined benchmark is listed, run or not"
    assert "bench.sched.hackbench" not in body, "a retired benchmark is not listed"
    assert "bench.made.up" not in body, "an unknown submitted id is not listed"
...
# --- GPU leaderboards group by the card, not the per-backend device name -----------
#
# A GPU benchmark row is keyed on the card's PCI id where ingest resolved it, so a card's Vulkan and
# OpenCL figures rank as one group, and two different cards a backend named the same stay apart.


def _intel_gpu_run(submitter, reviewer, *, device, value, run_id, device_id="7d55",
                   metric="vulkan_single_precision_compute"):
    inv = f.default_inventory()
    inv["summary"]["gpus"] = [{
        "pci": "0000:00:02.0",
        "pci_ids": {"vendor": "Intel Corporation [8086]",
                    "device": f"Intel Graphics [{device_id}]"},
        "driver": "i915",
    }]
    report = f.make_report(
        run_types=["benchmark"], run_id=run_id, inventory=inv,
        results=[f.benchmark_result(
            test_id="bench.gpu.clpeak", category="gpu",
            metrics=[{"name": metric, "value": value, "unit": "GFLOPS",
                      "direction": "higher_is_better", "device": device, "primary": True}],
        )],
    )
    run = ingest.ingest_bundle(
...
def test_a_cards_backends_rank_as_one_gpu_group(submitter, reviewer):
    """Same card named two ways: the group is keyed by its PCI id and labeled with the more
    specific name, with both runs counted, rather than split into two rows."""
    _intel_gpu_run(submitter, reviewer, device="Intel Graphics (ARL)", value=1000,
                   run_id="22222222-0000-0000-0000-000000000001")
    _intel_gpu_run(submitter, reviewer, device="Intel Graphics", value=900,
                   run_id="22222222-0000-0000-0000-000000000002")

    data = filters.leaderboard_groups(benchmark_id="bench.gpu.clpeak", group_by="gpu")

    assert len(data["groups"]) == 1, data["groups"]
    group = data["groups"][0]
    assert group["key"] == "8086:7d55"
    assert group["label"] == "Intel Graphics (ARL)"
    assert group["runs"] == 2


def test_two_different_cards_a_backend_named_alike_stay_apart(submitter, reviewer):
    """OpenCL reports a bare 'Intel Graphics' for every generation, so under the old device_model
    key an Arrow Lake and a Raptor Lake iGPU wrongly merged. Keyed on the PCI id they split, which
    is the same fix working the other direction."""
    _intel_gpu_run(submitter, reviewer, device="Intel Graphics", value=1000, device_id="7d55",
                   run_id="33333333-0000-0000-0000-000000000001")
    _intel_gpu_run(submitter, reviewer, device="Intel Graphics", value=500, device_id="a7a0",
                   run_id="33333333-0000-0000-0000-000000000002")

    data = filters.leaderboard_groups(benchmark_id="bench.gpu.clpeak", group_by="gpu")

    assert {g["key"] for g in data["groups"]} == {"8086:7d55", "8086:a7a0"}


def test_the_gpu_facet_offers_one_option_per_card_labeled_by_name(submitter, reviewer):
    """The facet dropdown lists a card once, its value the group key and its text a readable name,
    so choosing it filters by the card rather than by one backend's spelling of it."""
    _intel_gpu_run(submitter, reviewer, device="Intel Graphics (ARL)", value=1000,
                   run_id="44444444-0000-0000-0000-000000000001")
    _intel_gpu_run(submitter, reviewer, device="Intel Graphics", value=900,
                   run_id="44444444-0000-0000-0000-000000000002")

    facets = filters.leaderboard_facets("bench.gpu.clpeak", version=None)

    assert facets["gpu"] == [{"value": "8086:7d55", "label": "Intel Graphics (ARL)"}]
