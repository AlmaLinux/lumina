...
def test_no_family_picker_when_a_model_was_detected(submitter):
    """Offering both invites a contradiction: the family is derived from the
    model, so choosing one by hand could disagree with it."""
    run = _run(submitter)
    form = RunListingProposalForm(run=run, user=submitter)
    assert "cpu_model" in form.fields
...
def test_posting_the_form_stores_vendor_and_level_on_the_run(client, submitter,
    ...
    assert run.listing_proposal["cpu_model"] == "Xeon Gold 6430"
