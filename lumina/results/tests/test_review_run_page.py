...
def test_a_major_the_listing_lacks_is_a_real_declaration(submitter):
    ...
    assert effect["carried"] == []
    assert effect["new_declarations"] == [{"major": 10, "proved_in_queue": False}]
...
def _group_on_existing_listing(submitter):
    """An existing listing and a two-run group for it: run on 9 (ticking 10 too) plus a run on 10."""
    dell, _ = Vendor.objects.get_or_create(name="Dell Inc.", defaults={"published": True})
    System.objects.create(vendor=dell, name="PowerEdge R760")
    nine = _run(submitter, run_id="dddddddd-0000-0000-0000-000000000009", version_id="9.6")
    ten = _run(submitter, run_id="dddddddd-0000-0000-0000-000000000010", version_id="10.2")
    assert services.existing_listing_for(nine) is not None, "premise: re-validation of a listing"
    nine.listing_proposal = {"release_9": True, "release_10": True}
...
def test_a_queued_sibling_proving_a_claimed_major_is_not_dropped(submitter):
    """Reported: a group's second run adds AlmaLinux 10, but reviewing the first - on an existing
    listing - showed 10 as "Claimed but dropped", as if nobody was validating it, while the 10 run
    sat in the queue. It belongs in ``declares_queued``, not among the genuinely dropped."""
    nine, _ = _group_on_existing_listing(submitter)

    effect = services.proposal_effect(nine)

    assert effect["declares"] is False
    assert [d["major"] for d in effect["declares_queued"]] == [10]
    assert effect["declares_unproven"] == []


def test_the_page_credits_the_queued_run_on_an_existing_listing(client, submitter, reviewer):
    nine, _ = _group_on_existing_listing(submitter)

    prose = _prose(client, reviewer, nine)

    assert "Added by a queued run" in prose
    assert "adds it to this listing" in prose
    assert "Claimed but dropped" not in prose
