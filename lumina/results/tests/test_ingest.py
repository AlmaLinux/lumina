...
def test_ingest_collect_run(submitter):
    run = _ingest(f.make_report(run_types=["collect"]), submitter=submitter)

    assert run.run_type == RunType.collect.value
    assert run.status == TestRun.STATUS_PENDING
    assert run.published_at is None
    assert run.schema_version == "1.1"
    assert run.suite_version == "0.1.0"
