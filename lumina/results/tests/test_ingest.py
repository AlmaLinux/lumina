...
def test_combined_run_is_filed_as_validate(submitter):
    """A `alma-certify run` produces all three types; validation is the one that
...
def test_schema_1_0_report_still_accepted(submitter):
    ...
    A report can arrive without them - a collector that failed to classify, or one that never
    tried. Deriving it here classifies them on their merits instead: this
...
def test_legacy_gzip_bundle_still_accepted(submitter):
    """zstd is the format alma-certify emits now, but bundles produced by older
...
def test_submit_time_overrides_beat_report_values(submitter):
    """`alma-certify submit --pre-release --publish-after` must win, so an
...
def test_retarred_identical_report_is_still_a_duplicate(submitter):
    """`alma-certify submit` re-bundles the run dir on every invocation; archive
