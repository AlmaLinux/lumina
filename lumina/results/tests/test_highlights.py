...
FULL_RUN = [
    ...
    _row("bench.storage.fio-4k-randread", "storage"),
    _row("bench.storage.fio-seq-read", "storage"),
]
...
def test_a_disk_figure_never_headlines():
    """Storage benchmarks are retired; old rows still exist and must not lead."""
    shown, _ = headline_metrics(FULL_RUN)
...
def test_a_category_with_no_preference_falls_back_to_what_is_there():
    shown, _ = headline_metrics([_row("bench.odd.thing", "unheard-of")])
    assert [row.benchmark_id for row in shown] == ["bench.odd.thing"]
...
def _published_benchmark_run(submitter):
    report = f.make_report(
        ...
        results=[
            ...
            f.benchmark_result("bench.storage.fio-4k-randread", category="storage",
                               metrics=[
                {"name": "iops", "value": 132830.9, "unit": "IOPS",
                 "direction": "higher_is_better", "primary": True},
            ]),
...
def test_a_collect_only_run_is_not_in_the_benchmark_feed():
    ...
    report = f.make_report(
        run_types=["collect"], run_id="cccccccc-0000-0000-0000-000000000099",
        results=[],
    )
    run = ingest.ingest_bundle(
        submitter=submitter, bundle_file=f.as_upload(f.build_bundle(report)),
