...
def test_a_runs_own_gpu_columns_read_the_same_categorizer_as_its_device_table():
    """The facets and the device table must not disagree about what GPU a machine has.

    ``inventory_extract`` used to read ``summary["gpus"]`` directly, while the device
    table has always gone through the categorizer. Those two differ on any machine whose
    GPU collector failed while the PCI enumeration succeeded, so a run page could list an
    RTX card in its device table and report no GPU in its own columns, which then flowed
    into filtering and catalog matching. Same bug, same fix, one definition.
    """
    ...
    inventory = {"summary": {
        ...
        "gpus": [],          # the collector that failed
    }}

    columns = inventory_extract.extract(inventory)

    assert columns["gpu_model"] == "GA102 [GeForce RTX 3090]"
    assert columns["gpu_driver"] == "nvidia"
