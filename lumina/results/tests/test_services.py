...
def test_custom_build_run_attests_each_linked_component(
    ...
    # system (the board promoted to a machine) + board + matched CPU + the L40S GPU + Broadcom NIC
    assert CommunityAttestation.objects.filter(test_run=run).count() == 5
    assert CommunityAttestation.objects.filter(
        test_run=run, listing_component=board_listing
    ).exists()
    # The build now has a System listing, identified by its board, so it shows under Systems.
    from lumina.hardware.models import System
    system = run.listing_system
    assert system is not None
    assert system.name == board_listing.name and system.vendor == board_listing.vendor
    assert system.published is True
    assert CommunityAttestation.objects.filter(test_run=run, listing_system=system).exists()
    assert System.objects.filter(name=board_listing.name).count() == 1
...
def test_component_attestations_dedupe_per_submitter(
    ...
    # The board's System aggregates both builds: one System, attested once for this submitter.
    from lumina.hardware.models import System
    assert System.objects.filter(name=board_listing.name).count() == 1
    system = System.objects.get(vendor=board_listing.vendor, name=board_listing.name)
    assert system.attestation_count == 1
    assert CommunityAttestation.objects.filter(listing_system=system).count() == 1


def test_a_custom_build_appears_under_systems_with_its_parts(submitter, reviewer):
    """The reported gap: an approved custom build now has a System listing, identified by its
    board, published and shown under Systems, with the board/CPU/GPU tied on as a prebuilt's are."""
    ...
    assert system.published is True
    assert system in filter_listings(System, params={})   # shows in the Systems browse
    kinds = {c.kind for c in system.related_components.all()}
    assert "motherboard" in kinds and "gpu" in kinds
    assert system.cpus.exists()   # the CPU family attached to the certified-CPU set


def test_existing_listing_for_a_custom_build_resolves_its_boards_system(submitter, reviewer):
    """The machine listing for a custom build is the System its board identifies, so a
    re-validation's preview names that System rather than the board part."""
    from lumina.hardware.models import System
    ...
    services.approve_run(release(_custom_run()), by=reviewer)
    system = System.objects.get(name="B650M PG Riptide")

    second = _custom_run()
    # Clear the auto-link so resolution is exercised, not the direct FK: with no link, the machine
    # listing is resolved from the board, and it is the System, not the board part.
    second.listing_system = None
    second.listing_components.clear()
    second.save(update_fields=["listing_system"])
    assert services.existing_listing_for(second) == system
