...
# The GPU grouping key: the card's PCI id ("vendor:device") where ingest resolved it, and the
# device_model string where it did not (older rows, or a card that could not be pinned down). It is
# what makes a card's Vulkan and OpenCL figures - two device names, one PCI id - rank as one GPU,
# while a pre-PCI row still groups by its name exactly as before. device_model stays the human label
# and the family-matching input; only the grouping/facet KEY moves to this. Shared with compare.py.
GPU_GROUP_KEY = "device_group"


def with_gpu_group(qs):
    """Annotate ``GPU_GROUP_KEY`` onto a BenchmarkResult queryset.

    NullIf turns a blank device_pci_id into NULL so Coalesce falls through to device_model.
    """
    return qs.annotate(
        **{GPU_GROUP_KEY: Coalesce(NullIf("device_pci_id", Value("")), "device_model")}
    )


def representative_device_label(current: str | None, candidate: str) -> str:
    """The more specific of two device_model names for one grouped card.

    "Intel Graphics (ARL)" over the bare "Intel Graphics" that shares its PCI id: longest wins, ties
    broken alphabetically so the choice is deterministic across runs. Used by both the leaderboard
    and the compare picker so a card reads under the same name on both, rather than a plain SQL Max
    that would pick a bare "gfx1100" ROCm codename over the marketing name for an AMD card.
    """
    if current is None:
        return candidate
    return candidate if (len(candidate), candidate) > (len(current), current) else current
...
def benchmark_catalog() -> list[dict[str, Any]]:
    """Every benchmark lumina defines, with its public run count (0 if none submitted yet).

    Registry-driven, not submission-driven: the set is what lumina says should exist
    (``benchmark_registry``), so a retired benchmark whose old rows still sit in the database never
    appears here, and a defined benchmark nobody has run yet still does - as "0 runs".
    """
    counts = {
        row["benchmark_id"]: row["runs"]
        for row in (
            BenchmarkResult.objects.filter(run__in=TestRun.objects.public())
            .values("benchmark_id")
            .annotate(runs=Count("run", distinct=True))
        )
    }
    return [
        {"benchmark_id": b.id, "category": b.category, "runs": counts.get(b.id, 0)}
        for b in benchmark_registry.active_benchmarks()
    ]
    ...
    *, benchmark_id: str, params: dict | None = None
    ...
    for param, field in FACET_FIELDS.items():
        ...
        if not value:
            continue
        if param == "gpu":
            # ?gpu= carries the group key (a PCI id, or a device_model for an unresolved card), so
            # it is matched against the coalesced key, not the raw device_model column.
            qs = with_gpu_group(qs).filter(**{GPU_GROUP_KEY: value})
        else:
...
def leaderboard_facets(benchmark_id: str, version: str | None) -> dict[str, list]:
    ...
    for param, field in FACET_FIELDS.items():
        if param == "gpu":
            # The gpu facet is keyed on the group (a PCI id, or a device_model for an unresolved
            # card), so its options are {value: group key, label: representative device_model}
            # pairs rather than bare strings: the value drives ?gpu= and grouping, the label is what
            # a reader sees. Ordered by label.
            grouped: dict[str, str] = {}
            for row in with_gpu_group(base).exclude(device_model="").values(
                GPU_GROUP_KEY, "device_model"
            ):
                key = row[GPU_GROUP_KEY]
                if key in ("", None):
                    continue
                grouped[key] = representative_device_label(grouped.get(key), row["device_model"])
            facets["gpu"] = [
                {"value": key, "label": model}
                for key, model in sorted(grouped.items(), key=lambda kv: kv[1])
            ]
            continue
...
def median(values: list) -> float:
    ...
    Coerces to float before sorting so a mix of ``Decimal``/``int``/``float`` (and any numeric
    strings) orders numerically rather than by type or lexically.
    """
...
def leaderboard_groups(
    *, benchmark_id: str, params: dict | None = None, group_by: str | None = None
    ...
    # GPU groups key on the PCI-id-or-device_model group so a card's backends rank as one; the
    # visible label is a representative device_model, since the key may be a bare "8086:7d55". Every
    # other dimension keys and labels on the same string, so its label is just the key.
    is_gpu = group_by == "gpu"
    if is_gpu:
        rows = with_gpu_group(rows)
        value_fields = (GPU_GROUP_KEY, "device_model", "value", "unit")
    else:
        value_fields = (field, "value", "unit")

    buckets: dict[str, list] = {}
    labels: dict[str, str] = {}
    unit = ""
    for result in rows.values(*value_fields):
        key = result[GPU_GROUP_KEY] if is_gpu else result[field]
        if key in (None, ""):
        ...
        labels[key] = (
            representative_device_label(labels.get(key), result["device_model"])
            if is_gpu else key
        )
