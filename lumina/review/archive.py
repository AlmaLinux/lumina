...
# Per source, before the merge. A union at the database level would need seven models to share
# their columns and they do not, so the bound has to be applied before they are sorted together.
# Generous rather than tight, because this is the number that decides whether an old decision is
# reachable at all, and the page says so when it bites.
DECISION_LIMIT = 2000


def decisions(*, limit: int = DECISION_LIMIT) -> list[dict]:
    ...
    thousand rows is not worth the machinery, and ``limit`` is applied per source first so
...
def decision_text(row: dict) -> str:
    """One decision as the string a search matches against: what the table shows, and no more.

    Searching a column a reader cannot see finds rows for reasons nobody can account for.
    """
    return " ".join(str(part) for part in (
        row["kind"], row["subject"], row["outcome"], row["by"] or "", row["notes"],
    ))
