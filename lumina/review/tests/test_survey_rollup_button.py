...
# --- the facet rebuild, which the columns need and the rollup does not do ----------


def test_a_reviewer_can_rebuild_the_facet_columns(client, reviewer):
    """The columns segments filter on hold what a naming rule said when each machine reported.
    Correcting a rule fixes the page on the next rollup and leaves those behind, so there is a
    button for it beside the one that rebuilds the figures."""
    ...
    inventory = {"summary": {
        "cpus": [{"model": "AMD Ryzen 7 PRO 8840HS w/ Radeon 780M Graphics",
                  "vendor": "AuthenticAMD"}],
        "gpus": [{"pci": "64:00.0", "driver": "amdgpu",
    ...
    sub = SurveySubmission.objects.create(
        ...
        inventory=inventory, **extract.survey_extract(inventory, {}))
    SurveySubmission.objects.filter(pk=sub.pk).update(gpu_model="HawkPoint1")

    response = client.post(reverse("review:survey_facets_rebuild"), follow=True)

    assert response.status_code == 200
    sub.refresh_from_db()
    assert sub.gpu_model == "Radeon 780M"
    assert sub.inventory == inventory, "the payload is the raw layer and is never rewritten"


def test_the_button_is_on_the_survey_tab(client, reviewer):
    body = client.get(reverse("review:queue") + "?tab=survey").content.decode()

    assert reverse("review:survey_facets_rebuild") in body
    assert "Rebuild facet columns" in body


def test_only_a_reviewer_may_rebuild_the_columns(client):
    from django.contrib.auth.models import User

    client.force_login(User.objects.create_user("not-a-reviewer", password="pw"))

    assert client.post(reverse("review:survey_facets_rebuild")).status_code == 403
