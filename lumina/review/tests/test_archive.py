...
# --- the archive pages and searches -------------------------------------------------
#
# The decisions pane rendered every row it had, which is fine at ten decisions and a page nobody
# can read at a thousand.


def _archive(client, **params):
    return client.get(reverse("review:archive"), params)


def test_decisions_are_cut_into_pages(client, dell, submitter, reviewer):
    ...
    section = _archive(client).context["decisions"]

    assert len(list(section.page)) == 2
    assert section.count == 3


def test_a_second_page_of_decisions_is_reachable(client, dell, submitter, reviewer):
    from lumina.core.models import DisplayDefaults
    ...
    assert len(list(_archive(client, page_decisions="2").context["decisions"].page)) == 1


def test_decisions_can_be_searched(client, dell, submitter, reviewer):
    _decided_submission(dell, submitter, reviewer)

    found = _archive(client, q_decisions=reviewer.username).context["decisions"]
    missed = _archive(client, q_decisions="nobody-by-that-name").context["decisions"]

    assert found.count == 1
    assert missed.count == 0


def test_the_search_reads_what_the_table_shows(client, dell, submitter, reviewer):
    """Searching a column a reader cannot see finds rows for reasons nobody can account for."""
    _decided_submission(dell, submitter, reviewer)

    assert _archive(client, q_decisions="PowerEdge").context["decisions"].count == 1


def test_the_type_filter_and_the_search_hold_together(client, dell, submitter, reviewer):
    """Two controls on one pane, and applying the second must not drop the first."""
    _decided_submission(dell, submitter, reviewer)

    section = _archive(client, kind="Hardware listing", q_decisions="nobody").context["decisions"]

    assert section.count == 0
    assert "kind=Hardware+listing" in section.base


def test_the_activity_log_pages_at_the_configured_size(client, dell, submitter, reviewer):
    from lumina.core.models import DisplayDefaults

    DisplayDefaults.objects.update_or_create(pk=1, defaults={"rows_per_page": 1})
    _decided_submission(dell, submitter, reviewer)

    assert len(list(_archive(client).context["activity"].page)) <= 1


def test_the_activity_log_can_be_searched(client, dell, submitter, reviewer):
    """The pane that already paged and had only two fixed filters. A reviewer looking for what
    happened to one part had to know which action to pick from a dropdown of every action there
    has ever been."""
    from lumina.audit.services import log_action

    log_action("component_naming_rule.create", target=dell, actor=reviewer,
               notes="renamed a card")

    assert _archive(client, q_activity="naming_rule").context["activity"].count == 1
    assert _archive(client, q_activity="renamed a card").context["activity"].count == 1
    assert _archive(client, q_activity="no-such-action").context["activity"].count == 0


def test_the_cap_is_only_mentioned_when_it_bites(client, dell, submitter, reviewer):
    """A list that quietly stops at its cut-off is what the search box would be lying about."""
    _decided_submission(dell, submitter, reviewer)

    response = _archive(client)

    assert response.context["decisions_capped"] is False
    assert "Showing the most recent" not in response.content.decode()


def test_the_archive_renders_a_way_to_the_next_page(client, dell, submitter, reviewer):
    from lumina.core.models import DisplayDefaults
    ...
    body = _archive(client).content.decode()

    assert "page_decisions=2" in body
