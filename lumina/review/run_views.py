...
Validation runs and benchmark runs are separate queues (separate tabs in the
dashboard): validation runs are certification evidence, benchmark runs feed the
public leaderboards and are reviewed for plausibility. Collect-only runs are
hardware-survey submissions now, moderated in their own Survey tab.
"""
...
from lumina.results.models import RunType, TestResult, TestRun
from lumina.review.permissions import reviewer_required
from lumina.review.views import queue_redirect
...
def run_detail(request: HttpRequest, pk: int) -> HttpResponse:
    ...
    return render(
        ...
        "review/run_detail.html",
        {
            ...
            "verdict": run.verdict(),
            # Exactly what verdict() gates on, so a reviewer can see why approving this run will not
            # certify anything - and can waive a specific one to let it.
            "blocking_results": run.results.blocking(),
            # Failures a reviewer has already waived: shown as certified-despite, with who and why.
            "waived_results": run.results.waived(),
...
@reviewer_required
@require_POST
def run_waive_result(request: HttpRequest, pk: int) -> HttpResponse:
    """Waive one failing result so its run can certify despite it.

    ``pk`` is the result's, not the run's: a reviewer acts on a specific failure, and the run is
    reached through it for the redirect.
    """
    result = get_object_or_404(TestResult.objects.select_related("run"), pk=pk)
    try:
        services.waive_result(result, by=request.user, reason=request.POST.get("reason", ""))
        messages.success(request, f"Waived {result.test_id}.")
    except services.ReviewError as exc:
        messages.error(request, str(exc))
    return HttpResponseRedirect(reverse("review:run_detail", args=[result.run_id]))


@reviewer_required
@require_POST
def run_unwaive_result(request: HttpRequest, pk: int) -> HttpResponse:
    """Lift a waiver, so the failure blocks certification again."""
    result = get_object_or_404(TestResult.objects.select_related("run"), pk=pk)
    try:
        services.unwaive_result(result, by=request.user)
        messages.success(request, f"Removed the waiver on {result.test_id}.")
    except services.ReviewError as exc:
        messages.error(request, str(exc))
    return HttpResponseRedirect(reverse("review:run_detail", args=[result.run_id]))
...
@reviewer_required
@require_POST
def run_blacklist_device(request: HttpRequest, pk: int) -> HttpResponse:
    """Create a permanent PCI-device exclusion rule from the review screen.

    So a reviewer who never wants to see a part again (an onboard iGPU, a management NIC) blacklists
    it here rather than hand-typing its ids into the admin. The rule is *global* - it unticks this
    model by default on every future run - so it is audit-logged and easily undone in the admin.
    The current run is re-seeded at once, so the device shows excluded without waiting for a
    re-ingest.
    """
    run = get_object_or_404(TestRun, pk=pk)
    back = HttpResponseRedirect(reverse("review:run_detail", args=[run.pk]))
    if not blacklist_device(request):
        return back
    # Reflect it on this run immediately (seeding otherwise happens only at ingest).
    services.seed_rule_exclusions(run)
    return back


def blacklist_device(request: HttpRequest) -> bool:
    """Create the exclusion rule a blacklist form describes, and say whether it was usable.

    The rule is a PCI vendor and device id, which is all it ever needed: where a reviewer happened
    to be looking when they decided is not part of it. Shared so the survey page can offer the same
    action - the census counts the same devices the catalog does, and a reviewer looking at a
    survey submission had no way to say "never count this part" without one.
    """
    from lumina.hardware.models import ComponentExclusionRule, ComponentKind

    vendor_id = (request.POST.get("vendor_id") or "").strip().lower()
    device_id = (request.POST.get("device_id") or "").strip().lower()
    if not (vendor_id and device_id):
        messages.error(request, "That device has no PCI id to blacklist.")
        return False
    kind = (request.POST.get("kind") or "").strip()
    if kind not in {k.value for k in ComponentKind}:
        kind = ""
    reason = (request.POST.get("reason") or "").strip() or "excluded by a reviewer"

    rule, created = ComponentExclusionRule.objects.get_or_create(
        vendor_id=vendor_id, device_id=device_id, kind=kind,
        defaults={"reason": reason[:200]},
    )
    if created:
        log_action(
            "component_exclusion.create", target=rule, actor=request.user,
            after={"vendor_id": vendor_id, "device_id": device_id,
                   "kind": kind, "reason": rule.reason},
        )
        messages.success(
            request,
            f"Blacklisted {vendor_id}:{device_id}. It is unticked by default on every run now, "
            "and not counted in the statistics; undo it under Component exclusion rules.",
        )
    else:
        messages.info(request, f"{vendor_id}:{device_id} was already blacklisted.")
    return True


def _run_tab(run: TestRun) -> str:
    """The queue tab this run sits under.

    Validation and benchmark runs are two panes and two different decisions, so a
    reviewer working through one of them should be returned to it rather than to the
    first tab. See ``review.views.queue_redirect``.
    """
    return (
        "benchmark-runs" if run.run_type == RunType.benchmark.value
...
def run_approve(request: HttpRequest, pk: int) -> HttpResponse:
    ...
    return queue_redirect(_run_tab(run))
