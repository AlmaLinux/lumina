...
class _SampleListing(TypedDict):
    ...
    # The manufacturer's own product page. Worth carrying for the real certifications seeded
    # below: the detail template already links it, and a sample listing whose "Product specs"
    # goes nowhere teaches the reader that the field is decorative.
    vendor_spec_url: NotRequired[str]
...
# where they ship el8/el9/el10 packages and name enterprise Linux rebuilds generally, the
...
_ECOSYSTEM_SOFTWARE: list[_EcosystemSoftware] = [
    ...
    {"vendor": "Checkmk GmbH", "name": "Checkmk",
     "description": "Infrastructure and application monitoring. Names AlmaLinux "
                    "among its supported enterprise Linux distributions.",
    ...
    {"vendor": "MongoDB, Inc.", "name": "MongoDB Community Server",
     "description": "Document-oriented database with enterprise Linux packages.",
     "categories": ["Database"], "majors": [8, 9]},

    # --- Cloud & Virtualization ---
    {"vendor": "Docker, Inc.", "name": "Docker Engine",
     "description": "Container runtime and tooling, with el8, el9, and el10 "
                    "packages for enterprise Linux distributions.",
...
_SAMPLE_VENDORS: list[tuple[str, bool, str, str]] = [
    ...
    # The two manufacturers behind the real certifications seeded below. Supermicro is already
    # here; the catalog page calls them "Super Micro Computer, Inc." and this keeps the short
    # brand, which is what DMI reports and what the rest of this codebase is written against.
    ("Fsas Technologies",   True,  "https://www.fsastech.com",   ""),
    ("Intel",               True,  "https://www.intel.com",      ""),
    ("AMD",                 True,  "https://www.amd.com",        ""),
    ("Community-Submitted", False, "",                      ""),
]

# The processor families the sample systems below actually ship with, so the CPU facet and the
# "systems running this CPU" panel have something true to show. Named the way the vendors write
# them, which is not how ``hardware/0003_reference_data`` names the reference families a *run*
# rolls up to ("Intel Xeon 6"): these are catalog entries a human typed, and telling the two
# apart in the devstack is worth more than making them collide.
_SAMPLE_CPUS: list[dict] = [
    ...
    {"name": "Intel® Xeon® Scalable 4th Generation", "vendor": "Intel", "model_number": "", "description": "4th Generation Intel® Xeon® Scalable processors (Sapphire Rapids)"},
    {"name": "AMD EPYC™ 7003 Series",             "vendor": "AMD",   "model_number": "",   "description": "AMD EPYC™ 7003 Series processors with AMD 3D V-Cache™ Technology"},
]
...
class Command(BaseCommand):
    ...
    def _seed_sample_listings(self) -> None:
        """The real certifications, so the catalog is not empty and not made up on first visit.

        These are the four systems published at almalinux.org/certification/ecosystem-catalog,
        with their manufacturers, processors, releases, and product pages as that catalog states
        them. They replaced an invented Dell PowerEdge and Supermicro SuperServer, which had
        taught everyone reading a devstack that the catalog's shape was plausible without ever
        showing them what a real entry looks like.

        The tier is what the catalog says ran the tests, not who makes the hardware. Fsas
        Technologies ran their own, so those are vendor-validated; the two Supermicro systems say
        "Tests Run By: AlmaLinux OS Foundation", which is the almalinux tier here. Recording those
        as vendor-validated is the exact conflation ``results.services.effective_level`` exists to
        prevent - a Foundation certifier validating somebody's machine does not make it the
        manufacturer's word.
        """
        ...
        fsas = Vendor.objects.filter(slug="fsas-technologies").first()
        if not dell or not supermicro or not fsas:
            return
        ...
        samples: list[_SampleListing] = [
            {
                ...
                "vendor": fsas,
                # owner_vendor set, so Fsas submit-role members can propose edits through the
                # self-service flow - which is the arrangement a real certified listing has.
                "owner_vendor": fsas,
                "model_number": "RX2540 M8",
                "description": (
                    "Dual-socket 2U rack server for AI, business processing, graphics "
                    "rendering, and in-memory databases. Up to 32 DIMM modules (8TB DDR5), "
                    "12x 3.5-inch or 24x 2.5-inch storage devices, and 2 GPU cards."
                ),
                "vendor_spec_url": (
                    "https://eu.fsastech.com/eu/products-services/primergy-servers/"
                    "primergy-rx2540-m8/"
                ),
            ...
            },
            {
                ...
                "description": (
                    "Dual-socket 1U rack server for virtualization, scale-out deployments, "
                    "databases, and HPC. Up to 32 DIMM modules (8TB DDR5) and 4x 3.5-inch or "
                    "10x 2.5-inch storage devices."
                ),
                "vendor_spec_url": (
                    "https://eu.fsastech.com/eu/products-services/primergy-servers/"
                    "primergy-rx2530-m8/"
            ...
            },
            {
                ...
                "description": (
                    "2U four-node BigTwin, each node dual-socket AMD EPYC 7003/7002 with 16 "
                    "DIMMs, sharing 12 hot-swap 3.5-inch NVMe/SAS/SATA bays."
                ),
                "vendor_spec_url": (
                    "https://www.supermicro.com/en/Aplus/system/2U/2124/AS-2124BT-HNTR.cfm"
                ),
                "tags": [("Architecture", "x86_64")],
                "cpus": ["AMD EPYC™ 7003 Series"],
                # Certified on two majors at once, which is what a listing certified across
                # releases looks like - the per-release table on the detail page is the only
                # place that reads, and until now nothing seeded showed it.
                "releases": [(9, ValidationLevel.ALMALINUX), (8, ValidationLevel.ALMALINUX)],
            },
            {
                "model": System,
                ...
                "description": (
                    "2U CloudDC server with dual LGA-4677 sockets for 4th and 5th generation "
                    "Intel Xeon Scalable processors, 16 DIMMs up to 4TB DDR5, and 12 hot-swap "
                    "hybrid NVMe/SATA/SAS bays."
                ),
                "vendor_spec_url": (
                    "https://www.supermicro.com/en/products/system/clouddc/2u/sys-621c-tn12r"
                ),
                "tags": [("Architecture", "x86_64")],
                "cpus": ["Intel® Xeon® Scalable 4th Generation"],
                "releases": [(9, ValidationLevel.ALMALINUX), (8, ValidationLevel.ALMALINUX)],
            },
            {
                "model": Component,
        ...
        for s in samples:
            obj, created = s["model"].objects.get_or_create(
                name=s["name"],
                vendor=s["vendor"],
                # No tier and no count, for the reason ``_seed_sample_cpus`` gives: both are
                # *derived* columns, recomputed from the attestations written below before this
                # loop ends. Set here, they were a claim the spec could state and the evidence
                # could contradict - a sample could say "vendor-validated" while every release
                # row under it said otherwise, and nothing would notice.
                defaults={
                    "model_number": s["model_number"],
                    "description": s["description"],
                    "vendor_spec_url": s.get("vendor_spec_url", ""),
