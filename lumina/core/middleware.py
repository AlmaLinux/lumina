...
# django.contrib.admin, whose inline scripts and styles do not carry our nonce, so a
# ``script-src 'self' 'nonce-...'`` there would break the admin. It is staff-only and behind
# authentication, and the elimination of every third-party asset host (static/vendor) already
# removed the CDN-compromise vector for it too, so a looser script policy on this one prefix is an
# acceptable trade. Kept as a literal matching lumina/urls.py rather than reversed at request time.
_ADMIN_PREFIX = "/admin/"
