...
def test_the_django_admin_is_not_locked_out_by_the_strict_policy():
    ...
    # Unfold's Alpine.js evaluates its x-* directives via the Function() constructor, which CSP
    # gates behind 'unsafe-eval'. Without it every Alpine expression throws and the admin's modals,
    # sidebar, theme toggle, and command palette silently break.
