...
@admin.register(ComponentNamingRule)
class ComponentNamingRuleAdmin(ModelAdmin):
    """How a reported part becomes the name it is catalogued and counted under.

    Read this table before wondering why a card is called what it is: the answer is a row here,
    and the rule that named a part is recorded against it. Ordered by priority, lowest first, and
    the first rule producing a non-empty name wins.

    ``built_in`` rows reproduce judgements that used to be constants in code. They are editable,
    and an edit changes how every machine of that shape is named.
    """

    list_display = ("priority", "kind", "matches", "build", "built_in", "enabled")
    list_filter = ("kind", "enabled", "built_in")
    search_fields = ("model_pattern", "build", "notes")
    list_editable = ("enabled",)
    readonly_fields = ("created_at",)
    autocomplete_fields = ("created_by",)
    fields = ("kind", "match_field", "model_pattern", "conditions", "build", "fallback",
              "priority", "enabled", "built_in", "notes", "created_by", "created_at")

    @admin.display(description="Matches")
    def matches(self, obj) -> str:
        """The conditions in one line, so the changelist says what a rule applies to without
        opening it. They used to be three named columns, which is what made three of the hundreds
        of available fields look like the only three."""
        parts = [f"{obj.match_field or 'model'} matches {obj.model_pattern}"] \
            if obj.model_pattern else []
        parts += [f"{c.get('field')} {c.get('op', 'eq')} {c.get('value')}"
...
@admin.register(NameSuggestionField)
class NameSuggestionFieldAdmin(ModelAdmin):
    """Which of a machine's own reported values a submitter is offered when they say a part is
    misidentified.

    They pick a value, never a path: being asked which field holds the model is being asked to
    understand the payload. These rows decide which values are worth offering, and the label is
    what tells two similar ones apart.
    """

    list_display = ("kind", "path", "label", "priority", "enabled", "built_in")
    list_filter = ("kind", "enabled", "built_in")
    list_editable = ("enabled",)
    search_fields = ("path", "label")
    fields = ("kind", "path", "label", "priority", "enabled", "built_in")
