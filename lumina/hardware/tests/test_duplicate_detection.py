...
def test_the_queue_costs_the_same_however_many_rows_it_has(client, reviewer, dell):
    ...
    # One render before measuring, to warm the sidebar's queue-total cache. That total is
    # a fixed handful of COUNTs cached for a few seconds (lumina.review.queue_counts), so
    # a cold first render and a warm second one differ by a constant that has nothing to
    # do with how many rows are in the queue, which is what this test is about.
    queries()
