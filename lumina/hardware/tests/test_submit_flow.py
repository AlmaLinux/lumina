...
class CreateNewListingTests:
    ...
    def test_notification_email_sent_to_reviewers(self, client, submitter, dell):
        ...
        # Deferred: no mail in the request (the submission never blocks on SMTP), an outbox event
        # instead. The reviewers are mailed when the drainer runs.
        from lumina.notifications import services
        from lumina.notifications.models import NotificationEvent
        assert mail.outbox == []
        assert NotificationEvent.objects.filter(event_key="submission.created").exists()
    ...
    def test_release_compatibility_is_persisted(self, client, submitter, dell):
        ...
        AlmaLinuxRelease.objects.get_or_create(major=9)
        AlmaLinuxRelease.objects.get_or_create(major=10)
    ...
    def test_unchecked_release_is_not_attached(self, client, submitter, dell):
        ...
        alma9, _ = AlmaLinuxRelease.objects.get_or_create(major=9)
        AlmaLinuxRelease.objects.get_or_create(major=10)
