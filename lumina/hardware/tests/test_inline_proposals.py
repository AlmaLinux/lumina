...
from lumina.hardware.services import PUBLICATION_REFERENCE, publication_state
...
class InlineNewVendorOnSubmitTests:
    ...
    def test_inline_vendor_is_left_fully_unclaimed(self, alice, client):
        """Typing a vendor's name into a submit form is not evidence of representing them, so it
        grants nothing: no membership of any role, and the listing is left unowned. Ownership is
        what ``Vendor.is_claimed`` reads and what ``derive_allowed_levels`` turns into the vendor
        tier once verified, so a stranger who typed "Dell" must not end up holding Dell's identity.

        This used to grant ``ROLE_SUBMITTER`` on the reasoning that they needed it to edit their own
        listing. They do not: a bounced draft is fixed through ``revise`` (keyed on the submitter),
        and once published the listing is community-owned until a real ``VendorClaim`` takes it. The
        membership only put a community submitter in the vendor's roster and let them submit on its
        behalf - the coupling this separates out.
        """
        client.force_login(alice)
        client.post("/submit/", {
            "kind": "system",
        ...
        assert not VendorMembership.objects.filter(vendor=v).exists()
        assert v.is_claimed is False
        assert System.objects.get(name="Box C").owner_vendor_id is None
...
class InlineNewCpuOnSubmitTests:
    ...
    def test_inline_cpu_records_who_proposed_it(self, alice, existing_vendor, client):
        """The listing itself records its creator and this did not, so the two halves of one
        submission disagreed about where the CPU came from."""
        intel = Vendor.objects.get_or_create(name="Intel", defaults={"published": True})[0]
        client.force_login(alice)
        client.post("/submit/", {
            "kind": "system",
            ...
            "vendor": existing_vendor.slug,
            "claimed_validation_level": ValidationLevel.COMMUNITY,
            "new_cpu_name_0": "Attributed CPU Family",
            "new_cpu_vendor_0": intel.slug,
        })

        cpu = Component.objects.get(name="Attributed CPU Family")
        assert cpu.created_by == alice

    def test_the_proposer_is_not_told_their_own_cpu_is_somebody_elses(
            self, alice, existing_vendor, client):
        """What the missing creator actually did. ``publication_state`` reads "no creator" as
        one of the 81 seeded reference families, so the dashboard answered a submitter asking
        about the part they had just typed in by calling it a shared catalog entry, not a
        listing of theirs, with nothing to do about it.
        """
        intel = Vendor.objects.get_or_create(name="Intel", defaults={"published": True})[0]
        client.force_login(alice)
        client.post("/submit/", {
            ...
            "vendor": existing_vendor.slug,
            "claimed_validation_level": ValidationLevel.COMMUNITY,
            "new_cpu_name_0": "Mine CPU Family",
            "new_cpu_vendor_0": intel.slug,
        })

        state = publication_state(Component.objects.get(name="Mine CPU Family"), alice)

        assert state["kind"] != PUBLICATION_REFERENCE
        assert not state["shared"]
