...
# --- "I already approved the runs where these components were attached" --------------
#
# Reported from a dashboard: six components of an approved machine, every one of them
# reading "Nothing has validated this listing yet. Submit a passing validation run for
# it" - to the person who had just watched a reviewer approve exactly that. The state
# machine had no case for an approved run at all, so both of the states one can leave a
# listing in fell through to the sentence for a listing nothing is attached to.


def test_an_approved_run_is_never_told_to_submit_a_run(dell, owner):
    """The complaint, in one assertion. Approving a passing validation run is what
    publishes what it ties, so an unpublished listing with an approved run attached is a
    fault on our side - and asking the reader to do the thing they just did is the one
    answer that cannot be right."""
    component = Component.objects.create(
        vendor=dell, name="L40S", kind=ComponentKind.gpu.value, created_by=owner,
    )
    _run(owner, status=TestRun.STATUS_APPROVED, component=component)

    state = publication_state(component, owner)

    assert state["kind"] == PUBLICATION_STALLED
    assert "approved" in state["reason"]
    assert "Submit" not in state["next_step"], "they already did"
    assert "administrator" in state["next_step"]


def test_an_embargoed_run_says_it_is_being_held(dell, owner):
    """Approved and deliberately withheld is a normal state with a normal answer, and it
    was rendering as though nothing had ever been submitted."""
    listing = System.objects.create(vendor=dell, name="PowerEdge R750", created_by=owner)
    run = _run(owner, status=TestRun.STATUS_APPROVED, listing=listing)
    run.published_at = None  # approved, unreleased: TestRun.is_embargoed
    run.save(update_fields=["published_at"])

    state = publication_state(listing, owner)

    assert state["kind"] == PUBLICATION_WAITING
    assert "embargo" in state["reason"]
    assert "Nothing to do" in state["next_step"]


def test_an_embargo_with_a_date_says_the_date(dell, owner):
    listing = System.objects.create(vendor=dell, name="PowerEdge R750", created_by=owner)
    run = _run(owner, status=TestRun.STATUS_APPROVED, listing=listing)
    run.published_at = None
    run.publish_requested_date = timezone.now().date().replace(month=12, day=25)
    run.save(update_fields=["published_at", "publish_requested_date"])

    state = publication_state(listing, owner)

    assert run.publish_requested_date.isoformat() in state["next_step"]


def test_a_released_approval_beside_a_held_one_is_not_reported_as_held(dell, owner):
    """Only when *every* approved run is still under embargo is the embargo the answer.
    One released approval means something should have published and did not."""
    listing = System.objects.create(vendor=dell, name="PowerEdge R750", created_by=owner)
    held = _run(owner, status=TestRun.STATUS_APPROVED, listing=listing)
    held.published_at = None
    held.save(update_fields=["published_at"])
    _run(owner, status=TestRun.STATUS_APPROVED, listing=listing)

    assert publication_state(listing, owner)["kind"] == PUBLICATION_STALLED


def test_an_approval_outranks_a_rejected_sibling(dell, owner):
    """A machine run several times collects several runs. One of them being turned down
    says nothing once another was approved, and "the run was rejected, nothing about it
    will publish this" would send the reader after the wrong one."""
    listing = System.objects.create(vendor=dell, name="PowerEdge R750", created_by=owner)
    _run(owner, status=TestRun.STATUS_REJECTED, listing=listing)
    _run(owner, status=TestRun.STATUS_APPROVED, listing=listing)

    assert publication_state(listing, owner)["kind"] == PUBLICATION_STALLED


def test_a_shared_family_with_an_approved_run_is_still_flagged(dell, owner):
    """A seeded CPU family is not the reader's to publish, but "an approved run did not
    publish this" is true of it too, and it is how the fault gets noticed at all."""
    family = Component.objects.create(
        vendor=dell, name="Xeon Scalable 4th Generation", kind=ComponentKind.cpu.value,
    )
    _run(owner, status=TestRun.STATUS_APPROVED, component=family)

    state = publication_state(family, owner)

    assert (state["kind"], state["shared"]) == (PUBLICATION_STALLED, True)
