...
from lumina.core.certification import highest_level, level_outranks, resync_derived_levels
...
def record_attestation(*, version, listing, attested_by, level, source) -> bool:
    """Upsert the single counted attestation for ``(version, attested_by)``; return whether it was
    newly created (the number callers add up).

    Deduped per (release, person): a repeat by the same person on the same release stays one
    attestation, and a repeat carrying a higher tier upgrades their existing statement rather than
    adding a second - so the count reflects people, not runs. The evidence ``source`` is a
    ``TestRun`` or a ``Submission``, keyed onto its own FK; ``listing`` (a ``System`` or
    ``Component``) is denormalized onto the row as a query key. Shared by the run path
    (``results.services._attest_one``) and the submission path (``Submission.approve``), which is why
    the dedup-and-upgrade rule lives here rather than in either caller.
    """
    source_fk = {"submission": source} if isinstance(source, Submission) else {"test_run": source}
    attestation, created = CommunityAttestation.objects.get_or_create(
        version=version,
        attested_by=attested_by,
        defaults={**source_fk, "level": level, **listing_fk(listing)},
    )
...
def recompute_listing_levels(listing: HardwareListing) -> None:
    ...
    resync_derived_levels(versions)
...
def propose_listing_edit(
    *,
    ...
    proposal = ListingEditProposal.objects.create(
    ...
    emit("proposal.created", target=proposal, actor=proposed_by)
    return proposal
...
#
# Four different situations were rendering identically:
#
#   0. Already approved. An approved run is attached and the listing is still not published,
#      either because it is under embargo or because the approval did not record what it
#      should have. Reported from a dashboard showing six components of an approved machine:
#      "I already approved the runs where these components were attached." Telling that
#      reader to submit a passing validation run is telling them to do what they just did.
...
# An approved run is attached and this is still not published. Nothing the reader can do,
# and not a thing they should ever be asked to fix by running the suite again.
PUBLICATION_STALLED = "stalled"
...
def publication_state(listing: HardwareListing, user=None) -> dict | None:
    ...
    # Approved first, because an approved run is the strongest thing attached and both of the
    # states it produces used to fall all the way through to "nothing has validated this listing
    # yet. Submit a passing validation run for it" - told to a reviewer who had just approved one.
    #
    # Reaching here at all means the listing is unpublished despite that approval, since a
    # published listing returned None at the top.
    approved = [r for r in runs if r.status == TestRun.STATUS_APPROVED]
    if approved:
        held = [r for r in approved if r.is_embargoed]
        if len(held) == len(approved):
            when = next(
                (r.publish_requested_date for r in held if r.publish_requested_date), None,
            )
            return {
                "kind": PUBLICATION_WAITING,
                "shared": shared,
                "reason": (
                    "An approved run is being held under embargo, so this is not in the "
                    "catalog yet."
                ),
                "next_step": (
                    f"Nothing to do - it publishes on {when.isoformat()}." if when
                    else "Nothing to do - it publishes when the embargo is lifted."
                ),
            }
        # Approved, released, and still unpublished. Approving a passing validation run is what
        # publishes everything it is tied to, so this state is a fault rather than a step: it is
        # what an older ordering left behind, where certification was applied before the
        # components were tied and a listing's first-ever run therefore attested to nothing.
        # ``reapply_run_certification`` records what that approval should have.
        return {
            "kind": PUBLICATION_STALLED,
            "shared": shared,
            "reason": (
                "A run for this was approved, but it never published this listing. That is a "
                "fault on our side, not something left undone."
            ),
            "next_step": (
                "Nothing to do - an administrator can re-apply the approved run's certification."
            ),
        }
