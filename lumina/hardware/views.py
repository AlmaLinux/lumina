...
from lumina.core.certification import confirmations_count
...
def _filter_panel_categories(applies_to_scope: str) -> list[Category]:
    # The catalog's "Hardware" category
...
def _browse(request: HttpRequest, model: type[HardwareListing], kind: str) -> HttpResponse:
    ...
    ctx = {
        ...
        **_vendor_facet_context(request, model, kind),
        ...
        "socket_facets": _socket_facets() if kind == "systems" else [],
        "kind": kind,
        "component_kinds": [
            {"value": k.value, "label": k.label} for k in ComponentKind
        ],
        "active_filters": params(request),
        **_browse_seo(request, qs, kind),
    }
    template = (
...
def _listing_seo(request: HttpRequest, listing) -> dict:
    """What a listing page tells a search engine it is.

    This is the page the catalog exists to be found by - somebody searching for their own machine
    and the word AlmaLinux - so it gets a real description and a ``Product`` block rather than
    leaving a search engine to summarise the breadcrumb.
    """
    from lumina.core import seo

    url = request.build_absolute_uri(listing.get_absolute_url())
    releases = [
        f"{version.release.major}"
        for version in listing.versions.select_related("release").order_by("release__major")
    ]
    description = seo.listing_description(listing, releases)
    return {
        "page_title": f"{listing.name} - AlmaLinux Certification Catalog",
        "meta_description": description,
        "structured_data": seo.product_ld(listing, url, description),
    }


def _browse_seo(request: HttpRequest, listings, kind: str) -> dict:
    """What a browse page tells a search engine it is.

    The ``ItemList`` is capped: it exists so a crawler can see what a page holds and follow the
    entries, and the sitemap is what guarantees every listing is reachable. A block naming
    thousands of items would be most of the page's bytes for no gain.
    """
    from lumina.core import seo

    title = f"Certified {kind}"
    entries = [
        (listing.name, request.build_absolute_uri(listing.get_absolute_url()))
        for listing in listings[:_ITEM_LIST_LIMIT]
    ]
    return {
        "page_title": f"{title} - AlmaLinux Certification Catalog",
        "meta_description": seo.clamp(
            f"{title} for AlmaLinux: browse the {kind} verified by vendors, the community, or "
            "the AlmaLinux OS Foundation."
        ),
        "structured_data": seo.item_list_ld(title, entries),
    }


# Enough for a crawler to see the shape of the page and follow into it. The sitemap is what makes
# every listing reachable; this is not a second copy of it.
_ITEM_LIST_LIMIT = 50


def _socket_facets() -> list[int]:
    """Socket counts present across published systems, for the browse filter's checkboxes.

    Same derivation as ``System.socket_counts`` but pooled over the whole published catalog, so
    the filter only ever offers a value that some system will actually match.
    """
    from lumina.results.models import TestRun

    # order_by before distinct: see System.socket_counts for why (default Meta ordering would
    # otherwise defeat the single-column DISTINCT).
    return list(
        TestRun.objects.public()
        .filter(listing_system__published=True, cpu_sockets__isnull=False)
...
def _compatibility(listing):
    ...
    return (
        ...
        .annotate(attestation_total=confirmations_count())
        .order_by("-release__major")
    )


def _socket_context(listing) -> dict:
    """The socket configurations a System has been validated in, for the detail page."""
    if not isinstance(listing, System):
        return {}
    return {"socket_counts": listing.socket_counts()}
