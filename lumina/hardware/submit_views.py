...
def revise(request: HttpRequest, uuid: str) -> HttpResponse:
    ...
    if request.method == "POST" and form.is_valid():
        form.save()
        _notify_reviewers(submission, revised=True)
        messages.success(
            request,
...
def _notify_reviewers(submission, *, revised: bool = False) -> None:
    ...
    send_mail(
        subject=(
            f"[Lumina] {'Revised' if revised else 'New'} submission: {listing}"
        ),
        message=(
            f"A {'revised' if revised else 'new'} submission is awaiting review.\n\n"
            f"Listing: {listing}\n"
            f"Submitter: {submission.submitter}\n"
            f"Claimed level: {submission.get_claimed_validation_level_display()}\n"
        ),
