...
class SubmissionForm(forms.Form):
    ...
    def _resolve_inline_vendor(self) -> Vendor:
        """Materialize the inline-proposed vendor as a draft (published=False), fully unclaimed.

        No membership is granted and the caller leaves ``owner_vendor`` unset - typing a company's
        name is not standing to own it. See ``vendors.services.create_inline_vendor``; the submitter
        finishes a bounced draft through ``revise``, not through vendor rights.
        """
    ...
    def _create_listing(self, model: type[HardwareListing]) -> HardwareListing:
        ...
        # is left unclaimed - typing the name is not standing to own the listing. See
        # vendors.services.create_inline_vendor.
        owner = on_behalf
    ...
    def _attach_cpus(self, listing: HardwareListing) -> None:
        ...
        for name_f, vendor_f, desc_f in self._new_cpu_rows:
            ...
            cpu = Component.objects.create(
                ...
                published=False,
                # As on the listing itself. Left unset, a CPU somebody typed on this form was
                # indistinguishable from one of the 81 reference families the catalog seeds -
                # both have no creator - so their own dashboard told them the part they had
                # just proposed was "a shared catalog entry, not a listing of yours".
                created_by=self.user,
            )
            attach_cpu(listing, cpu)
