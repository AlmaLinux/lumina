...
def test_the_dashboard_says_how_to_run_the_suite(client, submitter):
    ...
    assert "sudo dnf -y install alma-certify &amp;&amp; sudo alma-certify" in body
...
def test_it_names_both_ways_a_result_gets_here(client, submitter):
    ...
    this site cannot submit for itself. Offering only the online path leaves that reader stuck."""
    body = _body(client, submitter)
...
def test_it_does_not_tell_anybody_to_register_first(client, submitter):
    """The run authorizes itself: it shows a device code before the tests start and submits when it
    finishes. A `alma-certify register` call to action described a step the suite already takes, and
    made a reader choose a path before they knew which one applied to them."""
    body = _body(client, submitter)

    assert "alma-certify register" not in body
