...
def missing_scope(token, scope: str) -> bool:
    """Whether ``token`` is a scoped API token that lacks ``scope``.

    A ``None`` token is session/cookie auth, which is not scope-limited, so it never misses a scope.
    The gate the write endpoints share (results ingest, the submission stub), so the scope rule and
    the None-means-session-auth convention are stated once rather than pasted per view.
    """
    return token is not None and not token.has_scope(scope)
...
class DeviceAuthRequest(models.Model):
    """One `alma-certify register` attempt, following RFC 8628 semantics.
...
class NotificationDefaults(models.Model):
    """What a person's notification preferences start as, before they say anything.

    One row, edited in the admin. The defaults used to be the ``default=`` on each field below,
    which meant changing them was a migration and a deploy, and meant an account that had never
    opened its settings page could not be reached at all by a channel added later.

    Read wherever a preference is asked for, so an account with no settings row of its own follows
    these rather than a constant: turning chat on here reaches everybody who has never expressed a
    preference, which is the point of it being a default rather than a seed.
    """
    ...
    @classmethod
    def load(cls) -> NotificationDefaults:
        """The one row, created with the built-in defaults the first time it is asked for."""
        return cls.objects.get_or_create(pk=1)[0]


class AccountSettings(models.Model):
    """Per-account preferences that outlive any one submission.

    Its own model rather than columns on ``User`` because Keycloak is the source of
    truth for identity and the local user row is a synced projection of it - anything
    stored there is at risk of being treated as replaceable. A row is created on demand by
    ``for_user`` from ``NotificationDefaults``, and an account with no row is read as following
    those defaults, so one that has never opened the settings page behaves exactly like one that
    saved them.
    """
    ...
    # The *default* for runs ingested from here on, not a live override of what is
    # already published. Anonymity is per run once a run exists (``TestRun.
    # publish_anonymously``), because the submitter has to be able to attribute one run
    # and not another - and because a switch that silently rewrote the attribution of
    # everything already public, in both directions, every time it was flipped is not a
    # preference, it is a hazard. The settings form offers applying it to existing runs
    # as a deliberate, separate act.
    publish_anonymously = models.BooleanField(
        default=False,
        help_text="List new runs as Anonymous when they are published.",
    )
    # Independently switchable, because they are different intrusions: a mail can wait in an
    # inbox and a chat message interrupts. What a new account starts with is
    # ``NotificationDefaults``, not the ``default=`` here: these two only apply to a row created
    # without consulting it, which ``for_user`` never does.
    email_notifications = models.BooleanField(
        default=True,
    ...
    chat_dm = models.BooleanField(
        default=True,
        help_text="Send a Mattermost direct message when something needs this user's "
                  "attention.",
    )
    updated_at = models.DateTimeField(auto_now=True)
    ...
    @classmethod
    def for_user(cls, user) -> AccountSettings:
        """This account's settings, creating the row from the site defaults if it has none.

        From the defaults rather than from the field defaults, so the settings page opens showing
        what this account is actually getting rather than what the model was written with.
        """
        ...
        return cls.objects.get_or_create(user=user, defaults={
            ...
            "chat_dm": defaults.chat_dm,
        })[0]

    @property
    def effective_chat_handle(self) -> str:
        """The chat username to address, or "" when this account takes no DMs.

        Always the AlmaLinux username. There was a ``chat_handle`` field beside this, on the
        reasoning that a Mattermost account spelled the same way need not be the same human, so
        somebody had to confirm the match. That is not the arrangement: both platforms sign in
        through the same account, so the names cannot disagree, and a field that can only ever be
        set wrong is a field to remove.
        """
        return self.user.get_username() if self.chat_dm else ""

    @classmethod
    def chat_handle_for(cls, user, defaults: NotificationDefaults | None = None) -> str:
        """The handle to DM for this user, or "" if they take none.

        An account with no row follows the site defaults, so a channel turned on for everybody
        reaches the people who have never opened their settings - who are most people. ``defaults``
        is passed in when a caller is asking about many users at once, to keep it one query.
        """
        settings_row = cls.objects.filter(user=user).first()
        if settings_row is not None:
            return settings_row.effective_chat_handle
        defaults = defaults or NotificationDefaults.load()
        return user.get_username() if defaults.chat_dm else ""

    @classmethod
    def publishes_anonymously(cls, user) -> bool:
        """Whether this account's default is to publish anonymously.

        Read-only and allocation-free on the common path: ingest asks this for every
        upload, and an account that has never touched its settings should not have a row
        written by the act of submitting a run.
        """
        return cls.objects.filter(user=user, publish_anonymously=True).exists()
