...
class AccountSettings(models.Model):
    ...
    stored there is at risk of being treated as replaceable. A row is created on demand
    by ``for_user``, so an account that has never opened the settings page behaves
    ...
    # Independently switchable, because they are different intrusions: a mail can wait in
    # an inbox and a chat message interrupts. Default on for email, which is what every
    # account already received before there was a setting; off for chat, which nobody has
    # consented to yet.
    ...
    # --- chat notifications -------------------------------------------------
    #
    # Off until the person says otherwise, and that is not timidity. The handle below
    # defaults to this account's username, which comes from Keycloak, and a Mattermost
    # account with the same spelling is not necessarily the same human. Sending on an
    # assumed match would mean mailing one person's submission status to a stranger, so
    # the match is confirmed once by the person it concerns rather than inferred.
    ...
    chat_handle = models.CharField(
        ...
        help_text="Mattermost username, without the @. Blank means the same as the "
                  "AlmaLinux username.",
    )
    ...
    def for_user(cls, user) -> AccountSettings:
        """This account's settings, creating the row the first time it is asked for."""
        return cls.objects.get_or_create(user=user)[0]

    @property
    def effective_chat_handle(self) -> str:
        """The Mattermost username to address, or "" when this account takes no DMs.

        Defaults to the AlmaLinux username, which is the point of the feature: the
        platform already knows who somebody is, so a person should not have to retype it.
        The confirmation is the checkbox, not the handle.
        """
        if not self.chat_dm:
            return ""
        return (self.chat_handle or self.user.get_username()).lstrip("@").strip()

    @classmethod
    def chat_handle_for(cls, user) -> str:
        """The handle to DM for this user, or "" if they take none."""
        settings_row = cls.objects.filter(user=user).first()
        return settings_row.effective_chat_handle if settings_row else ""
