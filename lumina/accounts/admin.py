...
from lumina.accounts.models import AccountSettings, ApiToken, NotificationDefaults
...
class AccountSettingsAdmin(ModelAdmin):
    ...
    list_display = ("user", "publish_anonymously", "email_notifications", "chat_dm",
                    "updated_at")
    list_filter = ("publish_anonymously", "email_notifications", "chat_dm")
    search_fields = ("user__username",)
    autocomplete_fields = ("user",)
    readonly_fields = ("updated_at",)


@admin.register(NotificationDefaults)
class NotificationDefaultsAdmin(ModelAdmin):
    """What a person gets before they say otherwise, and what everybody who never says gets.

    One row. Adding or deleting is refused rather than hidden, because a second row would be read
    by nothing and a missing one is recreated on the next read, and neither is worth explaining.
    """

    list_display = ("email_notifications", "chat_dm", "updated_at")
    readonly_fields = ("updated_at",)

    def has_add_permission(self, request) -> bool:
        return not NotificationDefaults.objects.exists()
