...
class AccountSettingsForm(forms.ModelForm):
    ...
    class Meta:
        ...
        labels = {
            "publish_anonymously": "Publish my runs anonymously",
            "email_notifications": "Email me when something needs my attention",
            "chat_dm": "Send me a Mattermost direct message as well",
            "chat_handle": "Mattermost username",
        }
        help_texts = {
            "publish_anonymously": (
            ...
            "email_notifications": (
                "A run that needs details, a reviewer asking for changes, a decision on "
                "something you submitted."
            ),
            "chat_dm": (
                "The same notices, as a direct message. Independent of email: have "
                "either, both, or neither."
            ),
            "chat_handle": (
                "Leave blank to use your AlmaLinux username. Set it if your Mattermost "
                "username differs, since a message can only be delivered to a name that "
                "matches."
            ),
        }
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        bootstrapify(self)
        # The username as the placeholder rather than the value, so the field stays blank
        # and keeps following the account if the username ever changes.
        user = getattr(self.instance, "user", None)
        if user is not None:
            self.fields["chat_handle"].widget.attrs["placeholder"] = user.get_username()

    def clean_chat_handle(self):
        # People type the @; Mattermost does not want it stored.
        return (self.cleaned_data.get("chat_handle") or "").strip().lstrip("@")
