...
INSTALLED_APPS = [
    ...
    # Sitemaps only; the Sites framework is deliberately not installed, and the sitemap views fall
    # back to the request's own host, which is the right answer for a single-host deployment.
...
MIDDLEWARE = [
    ...
    # Marks every response noindex unless this deployment is the one meant to be found.
    "lumina.core.middleware.SearchIndexingMiddleware",
    "django.contrib.sessions.middleware.SessionMiddleware",
    "django.middleware.common.CommonMiddleware",
    "django.middleware.csrf.CsrfViewMiddleware",
    "django.contrib.auth.middleware.AuthenticationMiddleware",
    # After AuthenticationMiddleware, which is what puts request.user there to ask.
    "lumina.core.middleware.PrivateResponseMiddleware",
...
# --- Email -------------------------------------------------------------------
#
# An authenticating, TLS-wrapped relay is the ordinary case in production - Amazon SES, and every
# other hosted submission service - and only host and port were read here, so credentials written
# into the environment were loaded by nothing. The symptom is quiet, because notifications are
# delivered out of band: SES refuses the unauthenticated connection, ``deliver_notifications``
# records the failure, retries five times, and marks the delivery failed. Nobody's page breaks and
# nobody gets an email.
EMAIL_BACKEND = env("EMAIL_BACKEND", "django.core.mail.backends.smtp.EmailBackend")
EMAIL_HOST = env("EMAIL_HOST", "localhost")
EMAIL_PORT = int(env("EMAIL_PORT", "25"))
# SES SMTP credentials, which are *not* AWS access keys: they are generated from an IAM identity
# in the SES console and only work for submission. Blank for a local relay that needs no login.
EMAIL_HOST_USER = env("EMAIL_HOST_USER", "")
EMAIL_HOST_PASSWORD = env("EMAIL_HOST_PASSWORD", "")
# STARTTLS on 587, or implicit TLS on 465. Mutually exclusive - Django refuses both at connect
# time, which on this deployment means inside the drainer, so ``core.checks`` refuses them at
# startup instead where a deploy can see it.
EMAIL_USE_TLS = env_bool("EMAIL_USE_TLS", False)
EMAIL_USE_SSL = env_bool("EMAIL_USE_SSL", False)
# Django's own default is None, meaning wait forever. That is survivable in a request, which has
# its own timeout above it, and not here: the drainer is a systemd timer firing every minute, so a
# relay that accepts a connection and never answers would stack runs up until something gave out.
...
# Whether search engines may index this deployment. **Off unless a deployment says otherwise**,
# because the cost of the two mistakes is not symmetric: a production site missing from Google for
# a day is a nuisance, while a staging copy indexed under the same content competes with
# production for its own queries and publishes test submissions as search results. One setting
# drives both ``robots.txt`` and the ``X-Robots-Tag`` header - robots.txt alone stops crawling but
# not indexing, so a URL linked from anywhere else can still surface as a bare title.
LUMINA_ALLOW_INDEXING = env_bool("LUMINA_ALLOW_INDEXING", False)
...
# A Mattermost incoming webhook this deployment owns, reconciled into an endpoint and a post on
# every reviewer event by ``manage.py configure_notifications``. Endpoints are rows an admin edits,
# which means a rebuilt database loses them; configuring one from the environment is what makes a
# dev host come back from a reset, and a production one come back from a restore, still talking to
# chat. The URL is the credential - anyone holding it can post to the channel - so it belongs in
# vault, not in an inventory. Blank configures nothing and leaves the admin's own endpoints alone.
LUMINA_MATTERMOST_WEBHOOK_URL = env("LUMINA_MATTERMOST_WEBHOOK_URL", "")
# Optional: post somewhere other than the channel the incoming webhook is locked to.
LUMINA_MATTERMOST_CHANNEL = env("LUMINA_MATTERMOST_CHANNEL", "")
...
# Device-authorization flow (alma-certify register).
