...
from lumina.notifications.tests.helpers import routed_endpoint
...
def _posted(kind, event_key="run.needs_details"):
    routed_endpoint(
        event_keys=[event_key], name="hook", url="https://hook.invalid/x", kind=kind,
...
def test_a_mattermost_message_is_rich(submitter):
    ...
    body = _posted(NotificationEndpoint.KIND_MATTERMOST)["body"]
...
def test_the_colour_says_what_kind_of_news_it_is(submitter):
    ...
    body = _posted(NotificationEndpoint.KIND_MATTERMOST, event_key="run.approved")["body"]
...
def test_the_generic_envelope_keeps_its_shape_and_signature(submitter):
    ...
    captured = _posted(NotificationEndpoint.KIND_GENERIC)
...
def test_the_generic_envelope_also_gets_the_facts(submitter):
    ...
    body = _posted(NotificationEndpoint.KIND_GENERIC)["body"]
...
def test_email_needs_no_payload_builder():
    """It is an endpoint now, but nothing is POSTed to it: the email path renders a template."""
    from lumina.notifications.models import NotificationEndpoint

    assert NotificationEndpoint.KIND_EMAIL not in payloads.BUILDERS


def test_every_posting_endpoint_kind_has_a_builder():
    """Email is excluded by name rather than by omission: it is an endpoint, nothing is POSTed to
    it, and a kind that falls through to the generic builder by accident would ship as JSON to a
    platform expecting something else."""
    posting = {
        kind for kind, _label in NotificationEndpoint.KIND_CHOICES
        if kind != NotificationEndpoint.KIND_EMAIL
    }

    assert posting <= set(payloads.BUILDERS), posting - set(payloads.BUILDERS)
...
# --- the route form has to expose the whole model --------------------------------


def test_the_policy_form_exposes_every_editable_field():
    """``Meta.fields`` is an explicit list, so a field added to the model is invisible in the
    admin until somebody remembers to name it. ``direct_messages`` on the old endpoint was added
    and missed exactly that way, and the direct-message feature was unconfigurable while looking
    finished."""
    from lumina.notifications.admin import NotificationPolicyForm
    from lumina.notifications.models import NotificationPolicy

    editable = {
        field.name for field in NotificationPolicy._meta.get_fields()
        if getattr(field, "editable", False) and not field.auto_created
    }

    assert editable <= set(NotificationPolicyForm.Meta.fields), (
        editable - set(NotificationPolicyForm.Meta.fields)
    )


def test_the_destination_inline_exposes_every_editable_field():
    """The other half of the same page: everywhere an event goes besides email."""
    from lumina.notifications.admin import PolicyDestinationInline
    from lumina.notifications.models import PolicyDestination

    editable = {
        field.name for field in PolicyDestination._meta.get_fields()
        if getattr(field, "editable", False) and not field.auto_created
    }

    assert editable - {"policy"} <= set(PolicyDestinationInline.fields)


def test_the_destination_inline_opens_with_no_blank_row():
    """A row prefilled with defaults reads as a destination somebody configured, and on a policy
    that mails one audience it said there was a second one that did nothing."""
    from lumina.notifications.admin import PolicyDestinationInline

    assert PolicyDestinationInline.extra == 0
