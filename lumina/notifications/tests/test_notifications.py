...
def test_a_webhook_is_posted_and_signed(submitter):
    endpoint = routed_endpoint(
        event_keys=["run.needs_details"], name="ops", url="https://hook.example/x",
    )
...
def test_a_mattermost_webhook_posts_a_chat_message_without_a_signature(submitter):
    routed_endpoint(
        event_keys=["run.needs_details"], name="mm", url="https://mm.example/hooks/abc",
...
def test_a_mattermost_endpoint_needs_no_secret(submitter):
    endpoint = NotificationEndpoint.objects.create(
        name="mm", url="https://mm.example/hooks/abc", kind=NotificationEndpoint.KIND_MATTERMOST,
    )
...
def test_a_failing_webhook_retries_then_gives_up(submitter):
    routed_endpoint(
        event_keys=["run.needs_details"], name="bad", url="https://bad.example/x",
    )
    _draft_run(submitter)
...
def test_the_policy_form_offers_registry_events_and_round_trips():
    from lumina.notifications import events
    from lumina.notifications.admin import NotificationPolicyForm

    # Choices come from the registry, not a freeform text box.
    assert {choice[0] for choice in NotificationPolicyForm().fields["event_key"].choices} == set(
        events.EVENTS
    )

    NotificationPolicy.objects.filter(event_key="run.submitted").delete()
    form = NotificationPolicyForm(data={"event_key": "run.submitted", "enabled": True})
    assert form.is_valid(), form.errors
    assert form.save().event_key == "run.submitted"

    # An unknown key is rejected, not silently stored.
    assert not NotificationPolicyForm(data={"event_key": "nope.nope"}).is_valid()
...
def test_an_endpoint_only_receives_the_events_it_lists():
    """The rule that surprised us: routing is explicit, so an event declared after the policies
    were configured reaches nobody. Auto-routing would change what a configured endpoint sends
    without anybody asking, so the table stays authoritative."""
    from lumina.notifications.models import NotificationEndpoint

    policy = NotificationPolicy.objects.get(event_key="run.submitted")

    assert [d.audience for d in policy.destinations.filter(
        endpoint__kind=NotificationEndpoint.KIND_EMAIL)] == ["reviewers"]
    assert not NotificationPolicy.objects.filter(event_key="nope.nope").exists()
...
def test_a_subscribed_endpoint_is_posted_a_survey_submission(submitter):
    ...
    endpoint = routed_endpoint(
        event_keys=["survey.submitted"], audience="reviewers",
        name="ops", url="https://example.invalid/hook", secret="s3cret", enabled=True,
    )
    _reset()
    posted = []
...
def test_the_admin_counts_what_points_at_an_endpoint():
    """"Why did that not fire" is answered here more often than anywhere else, and an endpoint
    nothing points at is configured and silent."""
    ...
    endpoint = routed_endpoint(
        event_keys=["run.submitted"], audience="reviewers",
        name="ops", url="https://example.invalid/hook", enabled=True,
    )
    PolicyDestination.objects.create(
        policy=policy_for("survey.submitted"), endpoint=endpoint, enabled=False,
    )

    assert NotificationEndpointAdmin.routes_using(None, endpoint) == "0 people, 1 rooms"
