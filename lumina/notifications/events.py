...
EVENTS: dict[str, Event] = {
    ...
    for e in [
        ...
        # The only transition that happens on a timer rather than from somebody's request, so
        # there is no page the submitter was watching when it occurred.
        Event("run.published", SUBMITTER, "Your run is now public",
              "An approved run came off hold and is now published.", tone=TONE_GOOD),
        ...
        Event("survey_submission.decided", SUBMITTER,
              "Update on your hardware survey submission",
              "A moderator accepted or dismissed a hardware survey submission.", tone=TONE_INFO),
        # --- edit proposals -------------------------------------------------
        ...
        # The answer to the one above. Reviewers were told a proposal had arrived and the person
        # who filed it was never told what came of it. Addressed at the proposer by default; a
        # route with the vendor_members audience sends it to the whole vendor instead.
        Event("proposal.decided", SUBMITTER, "Your proposed edit was reviewed",
              "A proposed correction to a catalog listing was approved or rejected.",
              tone=TONE_INFO),
    ]
}
