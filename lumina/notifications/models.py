...
class WebhookEndpoint(models.Model):
    ...
    direct_messages = models.BooleanField(
        default=False,
        help_text="Use this endpoint to send direct messages to individual users. "
                  "Mattermost only, and only where the incoming webhook is NOT locked to "
                  "a single channel, since a direct message overrides the channel.",
    )
...
class NotificationDelivery(models.Model):
    ...
    # The chat handle a direct message is addressed to, without the @. Blank for every
    # other channel. Kept beside ``email`` rather than reusing it because a chat handle is
    # not an address anybody can mail, and the two get confused the moment they share a
    # column.
