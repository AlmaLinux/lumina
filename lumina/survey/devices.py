...
from lumina.results.component_match import gpu_display_name, gpu_name_status
...
def _nics(obj) -> list[dict]:
    ...
    for dev in categorized_devices(obj).get("nics") or []:
        ...
        out.append({
            ...
            "link": dev.get("link"),
            "pci_vendor_id": pci_vendor_id(dev),
            "pci_device_id": pci_device_id(dev),
        })
    return out
...
def _gpus(obj) -> list[dict]:
    ...
    rules = active_rules()
    devices = [d for d in (categorized_devices(obj).get("gpus") or []) if isinstance(d, dict)]
    cpu_model = getattr(obj, "cpu_model", "") or ""
    for dev in devices:
        ...
        # The same status the run pages show, from the same function: a reviewer seeing a
        # transformed name on one page and the bare reported one on the other is the divergence
        # this whole area exists to remove.
        status = gpu_name_status(dev, cpu_model, devices)
        out.append({
            "vendor": vendor,
            "model": status["name"] or model,
            "naming": status,
            ...
            # The two things a reviewer can say about a part are both keyed on these, so every row
            # carries them whether or not the page happens to show them.
            "pci_vendor_id": pci_vendor_id(dev),
            "pci_device_id": pci_device_id(dev),
        })
    return out
...
def countable_gpus(inventory, rules: list | None = None) -> list[tuple[str, str]] | None:
    ...
    **All of them, not the most interesting one.** A workstation with an integrated Intel
    ...
    # The catalog's namer, not ``gpu_identity`` alone. This read lspci's string raw, so an AMD
    # APU was counted under its die codename ("HawkPoint1") while the review page beside it said
    # "Radeon 780M", and a discrete card was counted as "Navi 33 [Radeon RX 7600]" rather than the
    # product name. One machine had two names depending on which page you were on.
    cpu_model = ((summary.get("cpus") or [{}])[0] or {}).get("model") or ""
    out: list[tuple[str, str]] = []
    for gpu in devices:
        if census_exclusion_reason(gpu, rules) is not None:
            continue
        vendor, _raw = gpu_identity(gpu)
        pair = (_vendor_name(vendor), gpu_display_name(gpu, cpu_model, devices))
        if pair not in out:
            out.append(pair)
    return out
