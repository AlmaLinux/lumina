...
class SurveySubmissionQuerySet(models.QuerySet["SurveySubmission"]):
    ...
    def pending_review(self) -> SurveySubmissionQuerySet:
        ...
        Standalone survey runs only, because those are the only rows anybody can act on
        here. A cert-run fork is settled by the decision on the run it came from: approving
        that run accepts the fork, rejecting it dismisses the fork, and the survey pane
        offers no buttons for one.

        The count and the controls have to agree - a tab reading "1" beside three rows with
        working buttons is worse than either rule alone - so this and ``can_moderate`` are
        the same rule, one for a queryset and one for a row.
        """
        return self.filter(
...
class SurveySubmission(models.Model):
    ...
    ORIGIN_SURVEY = "survey"        # a standalone `alma-certify survey` run
    ...
    #
    # The facet columns are here because they are a derived index rather than raw data: they hold
    # what a naming rule said on the day the machine reported, and ``survey.facets.rebuild``
    # rewrites them from the payload when a rule is corrected. The payload itself, and the identity
    # read out of it, stay append-only - that is the part a rebuild must never be able to touch.
    ...
    # The certification run this was forked from, for a cert-run fork; null for a standalone
    # survey run. A string reference, so the census still imports nothing from the
    # certification side (see tests/test_firewall.py).
    #
    # Recorded because a fork is not moderated on its own: a reviewer judging the run judges
    # this with it, and approving or rejecting the run settles the fork. Without the link
    # there is nothing to settle it from, and the survey queue was asking a reviewer to make
    # the same call twice from two different pages.
    source_run = models.ForeignKey(
        "results.TestRun", null=True, blank=True,
        on_delete=models.SET_NULL, related_name="survey_forks",
    )
    ...
    def notification_fields(self) -> list[tuple[str, str]]:
        """Labelled facts for a rich chat notification. See ``TestRun`` for the contract.

        No serials and no identity hash: those are the access-controlled half of a
        submission, shown to a reviewer on the review page, and a chat channel is a wider
        audience than that page.
        """
        return [
            ("CPU", self.cpu_model),
            ("Board", self.board_model),
            ("Architecture", self.arch),
            ("AlmaLinux", f"{self.os_major}.{self.os_minor}"
                          if self.os_major and self.os_minor is not None else ""),
            ("Origin", self.get_origin_display()),
        ]

    @property
    def can_moderate(self) -> bool:
        """Whether a reviewer can accept or dismiss this submission from the survey queue.

        The row-level form of ``pending_review``, and the two must stay the same rule: the
        tab badge counts what this permits, and a count that disagrees with the buttons
        beside it is worse than either.

        A cert-run fork is never moderated here. It is evidence attached to a run that is
        reviewable in its own queue, and asking a reviewer to judge the same machine twice,
        from two pages, with no link between the decisions, is how the two ended up able to
        disagree.
        """
        return (
            self.review_state == self.REVIEW_NEW
            and self.origin == self.ORIGIN_SURVEY
        )

    @property
    def follows_run(self) -> bool:
        """Whether this submission's moderation is decided by a certification run."""
        return self.origin == self.ORIGIN_CERT_RUN
