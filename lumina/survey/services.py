...
from lumina.survey import extract, facets, identity
...
def _dimensions(sub: SurveySubmission, rules: list | None = None):
    ...
    Every one of them is derived from the stored payload by ``survey.facets``, and none is read
    off an extracted column. That is the point rather than an inefficiency: the columns hold what
    a rule said on the day a machine reported, so a rule corrected tomorrow would leave every
    historical row saying the old thing for ever. Deriving here means a rebuild corrects every
    submission ever made, including an admin adding an exclusion row that changes which devices
    count. It was half this way - GPUs derived, ``cpu_model`` read off the column - and the half
    that was not is how the statistics came to call a card "HawkPoint1" while its own run page
    called it "Radeon 780M".
    """
    for dimension, values in facets.display_facets(sub, rules).items():
        for value in values:
