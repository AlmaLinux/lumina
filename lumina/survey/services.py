...
def record_submission(*, inventory, environment, origin, trust_tier,
                      submitter=None, token=None, source_ip_hash="",
                      source_run=None) -> SurveySubmission:
    ...
    # Tell the reviewers, for the submissions that actually reach their queue. A cert-run
    # fork is not one: it is a byproduct of a validate or benchmark run that is reviewable
    # on its own, so mailing about it would double the traffic for every certification run
    # and say nothing new. ``emit`` writes one row inside this transaction and returns, so
    # an ingest that rolls back takes the notification with it.
    if origin == SurveySubmission.ORIGIN_SURVEY:
        emit("survey.submitted", target=submission, actor=submitter)
    return submission
    ...
    try:
        with transaction.atomic():
            return record_submission(
                ...
                # The link that lets the run's approval settle this fork.
                source_run=run,
            )
...
def _dimensions(sub: SurveySubmission, rules: list | None = None):
    ...
    """
    countable = devices.countable_gpus(sub.inventory, rules)
    ...
    # The brand, not the CPUID string the silicon answers with: nobody writes
    # "GenuineIntel". Applied here rather than at ingest so the stored column keeps what
    # the machine reported and a re-run corrects every submission already made, the same
    # arrangement as the GPU above. The table is the catalog's own, so a machine cannot be
    # Intel in the catalog and GenuineIntel in the statistics.
    yield "cpu_vendor", cpu_aliases.brand(sub.cpu_vendor)
    yield "cpu_sockets", str(sub.cpu_sockets) if sub.cpu_sockets else ""
    # One pair per GPU the machine has, not one per machine: see ``countable_gpus``. A
    # workstation with an integrated Intel display and a discrete NVIDIA card counts under
    # both, which is why these two dimensions are in MULTI_VALUED_DIMENSIONS and their
    # shares are of machines rather than of mentions.
    for vendor, model in countable:
        if vendor:
            yield "gpu_vendor", vendor
        cleaned = normalize.gpu_model(model or "")
        if cleaned:
            yield "gpu_model", cleaned
...
class NotDirectlyModeratable(ValueError):
    """Raised when something tries to moderate a cert-run fork on its own."""


def moderate_submission(submission: SurveySubmission, *, by, dismiss: bool,
                        via_run: bool = False) -> SurveySubmission:
    ...
    A cert-run fork is refused unless ``via_run``, which is how
    ``accept_forks_for_run`` marks the one caller allowed to decide it. The fork belongs
    to a run that is reviewable in its own queue; moderating it separately would let two
    pages disagree about the same machine.
    """
    if submission.follows_run and not via_run:
        raise NotDirectlyModeratable(
            "This submission came from a certification run and is decided with that run, "
            "not on its own."
        )
    ...
    refresh_period_for(submission)
    return submission


def accept_forks_for_run(run, *, by) -> int:
    """Mark the survey records forked from a run as reviewed, when the run is approved.

    Approval is the only run decision that reaches the census, and that asymmetry is
    deliberate. **A rejected run does not remove its machine from the figures.** The census
    asks what hardware exists and runs AlmaLinux, not what passed certification: a run
    rejected because the hardware failed a test, or because the listing details were wrong,
    still came from a real machine with a real inventory. Dropping it would quietly make
    the published counts a survey of *successful* certifications, which is a different and
    much less useful question. A fork from a rejected run therefore stays exactly where it
    was, counted and unreviewed.

    Isolated like the fork itself: a survey-side failure must never undo a certification
    decision that has already been made.
    """
    accepted = 0
    try:
        for fork in SurveySubmission.objects.filter(source_run=run):
            moderate_submission(fork, by=by, dismiss=False, via_run=True)
            accepted += 1
    except Exception:  # a census problem must not roll back a run decision
        logger.exception("could not accept survey forks for run %s", getattr(run, "uuid", "?"))
    return accepted


def refresh_period_for(submission: SurveySubmission) -> str | None:
    """Rebuild the month a submission counts in, so a moderation shows on the page at once.

    Dismissing is the decision that actually moves a number, since it removes the machine
    from the rollup. Accepting moves nothing on its own, because an unreviewed submission
    already counts: review here is oversight, not a gate. It still rebuilds, because a
    reviewer who has just acted should not have to wonder whether the page has caught up,
    and because the two decisions costing visibly different amounts of time would be its
    own kind of confusing.

    The submission's **own** month, not today's: moderating something from August has to
    fix August. Its year is left to the timers on purpose, because a yearly pass reads
    every submission of the year, which is far too much to do inside a reviewer's click.

    Never allowed to fail the moderation. The decision is recorded either way, and the
    hourly and nightly rebuilds will reach the same answer; a stale percentage for an hour
    is a much smaller problem than a moderation that appears to have failed.
    """
    period = timezone.localtime(submission.received_at).strftime("%Y-%m")
    try:
        rebuild_survey_stats(period=period)
    except Exception:  # a rollup problem must not undo a recorded decision
        logger.exception("survey stats refresh failed for period %s", period)
        return None
    return period
