...
# Dimensions where one machine can land in several buckets. A workstation with an
# integrated Intel display and a discrete NVIDIA card has both GPUs, and recording one of
# them meant recording the wrong one.
#
# The consequence is that their bucket counts do not add up to the machine count, so a
# share of the *sum* would answer "what fraction of GPU mentions were NVIDIA", which is a
# number nobody asked for and which moves when an unrelated machine gains a second card.
# They are divided by the machines in the period instead, making each share "the
# percentage of machines with at least one of these" - so they can total more than 100%,
# and the page says as much.
MULTI_VALUED_DIMENSIONS = frozenset({"gpu_vendor", "gpu_model"})
...
def _machine_counts(rows) -> dict[str, int]:
    """Machines counted per period, from the rollup's own count row."""
    return {
        row.period: row.count for row in rows
        if row.dimension == SurveyStat.MACHINES_DIMENSION
    }


def _denominator(period: str, dimension: str, totals: dict, machines: dict) -> int:
    """What a bucket's share divides by.

    The machines in the period for a multi-valued dimension, and the dimension's own total
    otherwise. Falls back to the total where no machine count is published, which is any
    period rolled up before that row existed: a slightly wrong denominator beats a
    division by zero that renders every share as 0%.
    """
    if dimension in MULTI_VALUED_DIMENSIONS:
        return machines.get(period) or totals.get((period, dimension), 0)
    return totals.get((period, dimension), 0)
...
def distribution(
    ...
    *,
    ...
    for dimension in DIMENSION_ORDER:
        ...
        if not current:
            continue
        total = _denominator(period, dimension, totals, machines)
        buckets = []
        ...
        sections.append({
            ...
            # So the page can say what the percentage is of, rather than leaving a reader
            # to wonder why these add up to more than a hundred.
            "multi_valued": dimension in MULTI_VALUED_DIMENSIONS,
...
def trend(
    ...
    *,
    ...
    if not rows:
        return None
    totals = _totals(rows)
    # The dimension-filtered query above cannot see the machine-count rows, so a
    # multi-valued dimension needs them fetched separately: without this its trend would
    # divide by the sum of its own buckets and read as a share of mentions.
    machines = (
        _machine_counts(
            SurveyStat.objects.filter(
                segment=segment, period__in=window, tier_scope=tier,
                dimension=SurveyStat.MACHINES_DIMENSION,
            )
        )
        if dimension in MULTI_VALUED_DIMENSIONS else {}
    )
    ...
    # Rounded up to a tidy gridline so the axis reads in tens, and never zero-height. A
    # single series cannot exceed 100% even where a dimension is multi-valued (a machine
    # counts once per bucket), so the ceiling still holds.
