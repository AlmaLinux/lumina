...
def survey_extract(inventory: dict, environment: dict) -> dict:
    ...
    cpu = inv.first_cpu(inventory)
    # Through the census's own helper, which applies the catalog's namer and the vendor alias
    # table. These columns are an index over what the statistics show, so they have to hold the
    # values the statistics show: they held the raw strings ("AuthenticAMD", "Advanced Micro
    # Devices, Inc. [AMD/ATI]", "HawkPoint1") while the page showed "AMD" and "Radeon 780M", so a
    # segment filtering on a vendor selected nothing at all. ``survey.facets`` decides what a value
    # is, and ``facets.rebuild`` rewrites these when that decision changes.
    counted = devices.countable_gpu(inventory) or ("", "")
    gpu_vendor, gpu_name = counted
    ...
    return {
        ...
        "cpu_vendor": _clean(
            cpu_aliases.brand(normalize.cpu_vendor(cpu.get("vendor") or "")), 80),
