...
def test_a_raw_column_cannot_ride_along_with_a_mutable_one():
    """``system_serial`` and not ``cpu_model``: the facet columns became a derived index that a
    facet rebuild rewrites from the payload, so they are mutable now. The payload and the identity
    read out of it are the raw layer, and are what this guard exists for."""
    sub = SurveySubmission.objects.create(
        origin=SurveySubmission.ORIGIN_SURVEY,
        trust_tier=SurveySubmission.TIER_VERIFIED,
    )
    sub.review_state = SurveySubmission.REVIEW_ACCEPTED
    sub.system_serial = "tampered"
    with pytest.raises(ValueError, match="append-only"):
        sub.save(update_fields=["review_state", "system_serial"])


def test_the_payload_itself_is_still_untouchable():
    """The line a rebuild must never cross: it rewrites derived columns and nothing else."""
    sub = SurveySubmission.objects.create(
        origin=SurveySubmission.ORIGIN_SURVEY,
        trust_tier=SurveySubmission.TIER_VERIFIED,
        inventory={"summary": {}},
    )
    sub.inventory = {"summary": {"cpus": [{"model": "invented"}]}}
    with pytest.raises(ValueError, match="append-only"):
        sub.save(update_fields=["inventory"])
