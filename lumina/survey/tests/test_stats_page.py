...
def test_cpu_vendors_are_published_as_brands_not_cpuid_strings(client):
    """A machine reports "GenuineIntel"; the page says Intel.

    Mapped in the rollup rather than at ingest, so the stored column keeps what the
    machine reported and a re-run corrects submissions already made. The table is the
    catalog's own (lumina.vendors.cpu_aliases), so a CPU cannot be Intel in the catalog
    and GenuineIntel in the statistics.
    """
    for i, vendor in enumerate(["GenuineIntel", "GenuineIntel", "AuthenticAMD"]):
        _sub(when=_at(2026, 9), identity_hash=f"c{i}", cpu_vendor=vendor)
    services.rebuild_survey_stats()

    response = client.get(reverse("results:stats"))
    section = next(s for s in response.context["sections"]
                   if s["dimension"] == "cpu_vendor")
    body = response.content.decode()

    assert [b.label for b in section["buckets"]] == ["Intel", "AMD"]
    assert "GenuineIntel" not in body
    assert "AuthenticAMD" not in body


def test_the_raw_cpuid_string_is_still_what_the_submission_records():
    _sub(when=_at(2026, 9), identity_hash="c", cpu_vendor="GenuineIntel")

    stored = SurveySubmission.objects.get(identity_hash="c")

    assert stored.cpu_vendor == "GenuineIntel", "the record says what the machine said"
