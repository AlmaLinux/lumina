...
def _start(client):
    resp = client.post(CODE_URL, {"client_name": "alma-certify 0.1.0"},
...
def test_activate_page_flow(client, approver):
    ...
    assert "alma-certify 0.1.0" in resp.text
...
def test_scanning_the_qr_link_shows_the_approval_screen_directly(client, approver):
    ...
    assert resp.status_code == 200
    assert "alma-certify 0.1.0" in resp.text          # the client being authorized is named
    assert body["user_code"] in resp.text
