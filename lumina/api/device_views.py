...
def device_code(request: Request) -> Response:
    ...
    client_name = str(request.data.get("client_name") or "alma-certify")[:80]
    device_request, raw_device_code = DeviceAuthRequest.start(
        client_name=client_name,
        hostname=str(request.data.get("hostname") or "")[:120],
        ip=ip,
    )
    verification_uri = request.build_absolute_uri(reverse("accounts:activate"))
    return Response(
        {
            "device_code": raw_device_code,
            "user_code": device_request.user_code,
            "verification_uri": verification_uri,
            # RFC 8628 verification_uri_complete: the same page with the code already in it, so a
            # client can render a QR the operator scans and approves without typing the code. The
            # activate view reads ?code= to prefill the form (accounts/views.py).
            "verification_uri_complete": (
                f"{verification_uri}?code={quote(device_request.user_code)}"
            ),
