...
class _BaseListingSerializer(serializers.ModelSerializer):
    ...
    def get_compatibility(self, obj) -> list[dict]:
        ...
        rows = (
            ...
            .annotate(confirmations=confirmations_count())
...
class SystemSerializer(_BaseListingSerializer):
    ...
    socket_counts = serializers.SerializerMethodField()

    class Meta(_BaseListingSerializer.Meta):
        model = System
        fields = _BaseListingSerializer.Meta.fields + ["cpu_support", "socket_counts"]

    def get_socket_counts(self, obj: System) -> list[int]:
        """Physical-socket configurations this system has been validated in. Documentation and a
        benchmark-context hint - socket count never gates certification, but results scale with
        it, so a dual-socket build is not comparable to a single-socket one."""
        return obj.socket_counts()
...
class BenchmarkResultSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ["benchmark_id", "benchmark_version", "category", "metric", "value",
                  ...
                  "device_pci_id", "device_ordinal", "context"]
...
class TestRunSerializer(serializers.ModelSerializer):
    ...
    # ``public_submitter_name``, so a run published without attribution reads as
    # "Anonymous" here too. This is the public API; the username is deliberately not
    # reachable through it.
    submitter = serializers.CharField(source="public_submitter_name", read_only=True)
...
class TestRunDetailSerializer(TestRunSerializer):
    ...
    # Scrubbed of system-identity fields (SMBIOS UUID, serials): the raw blob is
    # stored verbatim but must never reach the public. See ``public_inventory``.
    ...
    def get_inventory(self, obj: TestRun) -> dict:
        from lumina.results.inventory_extract import public_inventory

        return public_inventory(obj.inventory)
...
class LeaderboardRowSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            # device_pci_id ("vendor:device") is the per-card key the leaderboard groups on, so a
            # card's Vulkan and OpenCL rows group as one; device_model is its human label (blank id
            # falls back to it). gpu_model (the run's single card) stays for CPU rows and
            # back-compat. device_raw/ordinal disambiguate identical cards under --all-gpus. Blank/0
            # on non-GPU rows.
            "device_raw", "device_model", "device_pci_id", "device_ordinal",
