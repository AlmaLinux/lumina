...
class VendorClaim(ReviewWorkflow, models.Model):
    ...
    def approve(self, *, by, verify: bool, demote_others: bool = True) -> dict[str, int]:
        ...
        ``demote_others`` closes the squatting hole. Whoever created this vendor
        inline holds ROLE_SUBMITTER, and ``derive_allowed_levels`` grants the
        vendor tier to any submit-role member of a *verified* vendor - so
        verifying without demoting would let a stranger publish
        Vendor-validated content in this vendor's name. Off only when the
        reviewer can see the existing members are colleagues.
        """
