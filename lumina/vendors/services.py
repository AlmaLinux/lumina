...
def is_submit_member(user, vendor) -> bool:
    """Whether ``user`` holds a submit-role membership (submitter or owner) for ``vendor``.

    The single membership predicate behind "may act for this vendor", shared by every gate that
    asks it - level derivation, listing edits, vendor-profile edit proposals - so the role set and
    the query can never drift apart between them. ``vendor`` may be an instance or a pk (Django
    resolves ``vendor=<pk>``); an unauthenticated user never qualifies.
    """
...
def derive_allowed_levels(user, *, vendor: Vendor | None) -> list[str]:
    ...
    **The vendor tier is tied to attribution, never to standing.** It used to be
    ...
    if vendor is not None and vendor.verified and (
        certifying or is_submit_member(user, vendor)
    ):
...
def can_edit_listing(user, listing) -> bool:
    ...
    if listing.owner_vendor_id is None:
        return False
    return is_submit_member(user, listing.owner_vendor_id)


def vendors_for_submission(user, *, verified_only: bool = False) -> QuerySet[Vendor]:
    """Vendors ``user`` may submit on behalf of (submitter or owner role).

    ``verified_only`` narrows to verified vendors - the set for which naming the vendor actually
    unlocks the vendor tier, since ``derive_allowed_levels`` grants it only for a verified vendor.
    The run proposal's attribution picker wants that narrower set; the catalog submit forms, where
    naming an unverified vendor is still a valid community-tier attribution, want the full set.

    Accepts AnonymousUser (which has ``is_authenticated = False``) so callers don't need to guard
    the call site.
    """
    if not user.is_authenticated:
...
def can_propose_vendor_edit(user, vendor: Vendor) -> bool:
    ...
    """
    return is_submit_member(user, vendor)
...
def claim_vendor(
    *,
    ...
    emit("vendor_claim.submitted", target=claim, actor=requester)
    return claim
...
def create_inline_vendor(
    *,
    ...
    """Materialize a vendor a submitter named inline on a submission, fully unclaimed.

    Unpublished until a reviewer approves the submission that created it, so it stays out of the
    public catalog and out of other submitters' pickers.

    **No membership is granted, and the caller must not make this vendor the listing's
    ``owner_vendor``.** Typing a company's name into a form is not evidence of representing it, so
    the identity is left vacant for a ``VendorClaim`` to assign - exactly as a vendor materialized
    from a bundle's DMI is left unclaimed and memberless. The submitter needs no standing in the
    vendor to finish their own work: a bounced draft is fixed through the app's ``revise`` view,
    which is keyed on the submitter, and editing a *published* community listing is deliberately
    claim-gated (``can_edit_listing``) the same as everywhere else.

    This used to grant the creator ``ROLE_SUBMITTER`` and hand the listing to the new vendor, on the
    reasoning that they needed it to edit their own listing. They do not - ``revise`` covers the
    draft, and once published the listing is community-owned like any other - and the membership let
    a community submitter appear in the vendor's roster and submit on its behalf, which is the very
    coupling this separates out.
    """
...
def submit_vendor_with_claim(
    *,
    ...
    """Propose a brand-new vendor and claim ownership of it in one submission.

    The vendor is materialized unpublished and unclaimed - nothing public yet - and a ``VendorClaim``
    is opened against it. That claim is the single thing a reviewer acts on: approving it publishes
    the vendor, grants ownership, and carries the verify decision (``VendorClaim.approve``), so
    vetting the owner also vets the vendor. Rejecting it leaves an unpublished, unclaimed record
    that anyone else can still claim.

    Deliberately built out of the two existing pieces - ``create_inline_vendor`` and
    ``claim_vendor`` - rather than a parallel path, so the "unclaimed vendor" and "open claim"
    rules live in one place each.
    """
    vendor = create_inline_vendor(
        name=name, scope=scope, homepage=homepage,
        contact_email=contact_email, description=description, logo=logo,
    )
    return claim_vendor(
        vendor=vendor, requester=requester, work_email=work_email,
        role_at_vendor=role_at_vendor, note=note, evidence=evidence,
    )
