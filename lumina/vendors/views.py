...
from lumina.vendors.services import (
    ...
    claim_vendor,
    submit_vendor_with_claim,
)

_CLAIM_FIELDS = ("work_email", "role_at_vendor", "note", "evidence")


@login_required
def propose_new(request: HttpRequest) -> HttpResponse:
    """Submit a new vendor for review, optionally claiming ownership of it.

    Unticked, this writes a plain ``VendorProposal`` a reviewer approves into a published, unclaimed
    vendor. Ticked, the claim inputs are gathered and the submission becomes a ``VendorClaim`` on a
    freshly created (unpublished) vendor - one review that decides both whether the vendor is real
    and whether this person owns it. Both paths reuse existing services; nothing here re-implements
    vendor creation or the claim rules.
    """
    claiming = request.method == "POST" and bool(request.POST.get("claim_ownership"))
    if request.method == "POST":
        form = VendorCreateProposalForm(request.POST, request.FILES)
        claim_form = (
            VendorClaimForm(request.POST, request.FILES) if claiming else VendorClaimForm()
        )
        if form.is_valid() and (not claiming or claim_form.is_valid()):
            if claiming:
                claim = submit_vendor_with_claim(
                    ...
                    **{name: claim_form.cleaned_data[name] for name in _CLAIM_FIELDS},
                )
                messages.success(
                    request,
                    f"{claim.vendor.name} submitted with your ownership claim. A reviewer will "
                    "verify it before it is published.",
                )
            else:
        ...
        claim_form = VendorClaimForm()
    return render(
        request, "vendors/propose_new.html", {"form": form, "claim_form": claim_form}
    )
