...
def test_inline_vendor_creation_does_not_hand_over_ownership(stranger):
    """The whole point of the claim flow is that ownership is still available."""
    from lumina.vendors.services import create_inline_vendor

    vendor = create_inline_vendor(
        name="Acme", created_by=stranger, scope=Vendor.SCOPE_SOFTWARE,
    )

    membership = VendorMembership.objects.get(user=stranger, vendor=vendor)
    assert membership.role == VendorMembership.ROLE_SUBMITTER
    assert not vendor.is_claimed


def test_an_inline_vendor_starts_unpublished_and_unclaimed(stranger):
    from lumina.vendors.services import create_inline_vendor

    vendor = create_inline_vendor(name="Acme", created_by=stranger)
