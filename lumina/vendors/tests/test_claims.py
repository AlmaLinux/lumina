...
@pytest.fixture
def inline_acme(squatter):
    """Acme as a stranger left it: unpublished, unverified, unclaimed."""
...
def test_approval_demotes_the_squatter_who_created_the_vendor(inline_acme, squatter,
                                                              acme_engineer, reviewer):
    """The hole this closes: the inline creator holds ROLE_SUBMITTER, and
    derive_allowed_levels grants the vendor tier to any submit-role member of a
    verified vendor. Verifying Acme without demoting them would let a stranger
    publish Vendor-validated content as Acme.
    """
...
def test_demotion_can_be_declined_by_the_reviewer(inline_acme, squatter,
                                                  acme_engineer, reviewer):
    """Sometimes the existing member is a colleague, not a squatter."""
    claim = _claim(inline_acme, acme_engineer)
