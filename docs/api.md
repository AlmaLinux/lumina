...
- `sockets=<n>` (repeatable, systems only) - filter to systems validated in that
  physical-socket configuration; OR within. See `socket_counts` below.
...
System listings also carry `socket_counts`: the physical-socket configurations
the system has been validated in, ascending, derived from what its public runs
cited.
...
Socket count is not a certification dimension: a run in any configuration
validates the whole system, so this is documentation, not a claim. It matters
for benchmarks, which scale with socket count, so a dual-socket build is not
comparable to a single-socket one. Filter the systems list by it with
`?sockets=2` (repeatable, OR semantics); a system matches if any of its public
runs cited that count. An empty list means no run has cited a socket count yet.
...
The alma-certify suite's report schema lives in the suite repository
(`docs/schema.md` there); Lumina validates `schema_version: "1.0"`.

### `POST /api/v1/results/`

Ingest a result bundle (the `.tar.zst` produced by `alma-certify bundle`, and
what `alma-certify submit` POSTs). Compression is sniffed from magic bytes:
...
alma-certify refuses to upload such a run in the first place, so anything reaching
...
RFC 8628-shaped flow used by `alma-certify register`, so the headless machine
under test never sees a password:

### `POST /api/v1/device/code`

Body `{client_name}` → `{device_code, user_code, verification_uri,
verification_uri_complete, expires_in, interval}`. Anonymous, heavily
throttled, max 3 pending requests per IP.

`verification_uri_complete` is the RFC 8628 field: `verification_uri` with the
code already in it (`?code=<user_code>`). The activation page reads that
`code` param to prefill the form, and a client can render the complete URI as a
QR so the operator scans it, signs in if needed, and only clicks **Authorize** -
the client name and requesting IP are still shown for confirmation first.
...
`alma-certify` calls this in its **pre-run** checks. A token's stored `expires_at`
