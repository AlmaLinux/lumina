...
Optional secrets, same page. Each one is a feature the host does without when it is unset:

| Secret | What it is |
|---|---|
| `DEV_MATTERMOST_WEBHOOK_URL` | an incoming-webhook URL. Set it and the deploy configures the chat endpoint and points every reviewer event at it. See [Chat notifications](#chat-notifications) |
| `SENTRY_DSN` | error monitoring. Unset disables it |

Variables, under the same page → Variables. All optional:
...
| `DEV_MATTERMOST_CHANNEL` | post somewhere other than the channel the incoming webhook is locked to. A channel name as it appears in the URL bar |
...
Notification endpoints are database rows, so a reset takes the Mattermost endpoint with everything
else and the host comes back mailing but silent in chat. Setting `DEV_MATTERMOST_WEBHOOK_URL` is
what fixes that: the deploy runs `manage.py configure_notifications`, which creates the endpoint
and a post on every event a reviewer has to act on. It runs after the reset's migration for exactly
that reason, and on ordinary deploys too, so rotating the secret is a redeploy and nothing else.

Reviewer events only. A channel is a room, and "your run was rejected" is for its submitter; the
personal events stay on email and direct messages. An admin can add more in Django admin, and a
later deploy leaves what they add alone, including an endpoint they have switched off.

The URL is the credential - anyone holding it can post into the channel - so it is a secret, not a
variable, and nothing in the deploy prints it.
