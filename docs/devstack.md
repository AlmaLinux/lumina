...
The migration history was squashed to a single initial state for the 1.0 release - one
initial migration per app plus the seeds a clean install needs - so a production database
starts from exactly that graph. It strands any database created before the squash: its `django_migrations` table names migrations that no longer exist
