...
def test_approving_a_run_actually_submits(page, visit, sign_in, reviewer, pending_run):
    ...
    # ``?tab=...``: a decision returns to the tab it was made on, so the queue URL is no
    # longer bare. Matching the query too, since landing on the right tab is the point.
    page.wait_for_url("**/review/?tab=validation-runs")
